...
local dragPanel = UI.createPanel("DragIconPanel", Defines.UIGroup.PAGameUIGroup_ModalDialog)
...
function	DragManager:itemDragMove( toType, toActorKey )
	...
	if	( CppEnums.MoveItemToType.Type_Player ~= fromMoveType )	then
		if	( self.fromActorKeyRaw == toActorKey )	then
			return
		end
	end

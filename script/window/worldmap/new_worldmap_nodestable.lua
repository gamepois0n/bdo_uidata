...
function StableOpen_FromWorldMap( waypointKey )
	...
	if ( Panel_NodeGuildWarMenu:GetShow() ) then
		Panel_NodeStable		:SetPosX( Panel_NodeGuildWarMenu:GetPosX() + Panel_NodeGuildWarMenu:GetSizeX() + 10 )
	...
	self._nodeWaypointKey	= nil
	self._nodeServantCount	= 0
	
	self._nodeWaypointKey	= waypointKey
	self._nodeServantCount	= stable_countFromWaypointKey( self._nodeWaypointKey )

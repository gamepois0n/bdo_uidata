...
local WorkerTooltip_Index = nil
local ItemTooltip_Index = nil
...
function Control_WorkerList:SetControl(waitWorkerCount, maxWorkerCount)	
	...
	--btnControl._Recovery			:ComputePos()
	...
	for workerIndex = 0 , maxWorkerCount - 1  do
		...
		if workerIndex < waitWorkerCount then
			...
			if isWaitWorker(workerData) == true then
				Worker_Table[workerIndex]._State_Text = "<PAColor0xfffeff99>" .. PAGetString( Defines.StringSheet_GAME, "LUA_WORLD_MAP_TOWN_WORKER_RESTORE" ).."<PAOldColor>"		-- 회복
				Worker_Table[workerIndex]._State = eWorkerStateType.waiting -- 대기중
			else
				if  (workerData) == true then
					...
					Worker_Table[workerIndex]._State = eWorkerStateType.transfer -- 전출 중
				else
					Worker_Table[workerIndex]._State_Text = "<PAColor0xffde3900>" .. PAGetString( Defines.StringSheet_GAME, "LUA_WORLD_MAP_TOWN_WORKER_CANCELWORK" ).."<PAOldColor>"		-- 작업취소
					Worker_Table[workerIndex]._State = eWorkerStateType.working -- 작업중
				end
			end
		else
...
-- local workedWorker = {}
-- local reCall_RedoWork = false

-- function Recall_RedoWork()
-- 	return reCall_RedoWork
-- end
...
-- 	workedWorker[_selectedWorker:get_s64()] = {
-- 		_workType			= _workType,
...
-- function HandleClick_ReDoAllWork()
...
-- 	for workerIndex = 0 , maxWorkerCount - 1  do
-- 		if workerIndex < waitWorkerCount then
-- 			local workerNo = Worker_Table[workerIndex]._WorkerNo:get_s64()

-- 			local workedWorkerData = nil
-- 			for key, value in pairs(workedWorker) do
-- 				if ( key == workerNo ) then
-- 					workedWorkerData = value
-- 					break
-- 				end
-- 			end
-- 			if nil ~= workedWorkerData then
-- 				if 0 == workedWorkerData._workType then			-- 렌드 하우스 제작
-- 					ToClient_requestStartRentHouseWorkingToNpcWorker( workedWorkerData._houseInfoSS, workedWorkerData._selectedWorker, workedWorkerData._workKey, workedWorkerData._workingCount, workedWorkerData._itemNoRaw )
						
-- 				elseif 2 == workedWorkerData._workType then		-- 채집
-- 					ToClient_requestStartPlantWorkingToNpcWorker( workedWorkerData._selectedWorker, workedWorkerData._plantKey, workedWorkerData._workKey, workedWorkerData._workingCount )
						
-- 				elseif 4 == workedWorkerData._workType then		-- 파이낸스
-- 					ToClient_requestStartPlantWorkingToNpcWorker( workedWorkerData._selectedWorker, workedWorkerData._plantKey, workedWorkerData._workKey, workedWorkerData._workingCount )
						
-- 				elseif 1 == workedWorkerData._workType then		-- 대규모 제작
-- 					ToClient_requestStartLargeCraftToNpcWorker( workedWorkerData._houseInfoSS, workedWorkerData._selectedWorker, workedWorkerData._workKey, workedWorkerData._selectedSubwork, 1 )
						
-- 				elseif 3 == workedWorkerData._workType then		-- 성채/지휘소
-- 					requestStartHouseholdBuildingToNpcWorker( workedWorkerData._selectedWorker, workedWorkerData._plantKey, workedWorkerData._houseHoldNo, workedWorkerData._selectedSubwork, workedWorkerData._workingCount )
...
-- function HandleClick_ReDoWork( workerIndex )
-- 	local workerNo = Worker_Table[workerIndex]._WorkerNo:get_s64()
-- 	for key, workedWorkerData in pairs(workedWorker) do
-- 		if ( key == workerNo ) then
-- 			if 0 == workedWorkerData._workType then			-- 렌드 하우스 제작
-- 				ToClient_requestStartRentHouseWorkingToNpcWorker( workedWorkerData._houseInfoSS, workedWorkerData._selectedWorker, workedWorkerData._workKey, workedWorkerData._workingCount, workedWorkerData._itemNoRaw )
...
function Control_WorkerList:UpdateSlot(_Index, isAdjust)
	...
	for idx = 0, scroll_Param._slotMax - 1 do
		if nil ~= Worker_Table[workerIndex] and ( true ~= isAdjust or idx < _Index or _Index + adjust <= idx ) then
			if false == Worker_Table[workerIndex]._isEmpty then
				...
				-- self._worker_State[idx]			:SetText(  ) -- "취소" )
				
				if true == waitingCancelWorkerNo[workStringNo] then	-- todo:작업 취소를 여러군데에서 할 수 있다! 보완이 필요
					if Worker_Table[workerIndex]._State == eWorkerStateType.working then
						self._worker_State[idx]			:SetText( PAGetString(Defines.StringSheet_GAME, "LUA_NEW_WORLDMAP_TOWN_WORKERMANAGE_CANCELRESERVE") ) -- "취소예약" )
				...
				if ( nil ~= _workerData ) then
					...
					if ( 0 < _leftWorkCount ) then
						self._worker_Name[idx]:SetText( PAGetStringParam2( Defines.StringSheet_GAME, "LUA_WORLDMAP_TOWN_WORKERMANAGE_LEFTWORKCOUNT", "workerName", workerName, "_leftWorkCount", _leftWorkCount ) ) -- workerName .. " (" .. _leftWorkCount .. "회 남음)")
					end
				end
	...
	if _offsetIndex > _offsetMax then
...
function WorkerManage_CancelWork(workerIndex)	-- 1회면 취소 못하게
	...
	if _leftWorkCount < 1 then
		...
		local workName 	= PAGetString(Defines.StringSheet_GAME, "LUA_NEW_WORLDMAP_TOWN_WORKERMANAGE_PORTRESS") -- "성채"
		if nil ~= esSSW then
			workName 	= esSSW:getDescription()
		end
		local cancelWorkContent = PAGetStringParam1( Defines.StringSheet_GAME, "LUA_NEW_WORLDMAP_TOWN_WORKERMANAGE_CONFIRM_WORKCANCEL", "workName", workName ) -- "[" .. workName .. "]\n예약된 작업을 취소하시겠습니까?\n현재 진행중인 작업은 취소할 수 없습니다.\n\n※ 작업에 사용된 아이템은 복구되지 않습니다."
		-- local cancelWorkContent = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_WORKINGPROGRESS_CANCELWORK_CONTENT", "workName", workName)
...
function WorkerManage_CancelWork_Continue()
	...
	-- local isSuccess = ToClient_requestStopPlantWorking(_workerNo)
...
function HandleLClick_WorkerState_Button(workerIndex, idx)	-- 1회면 취소 못함!
	if Worker_Table[workerIndex]._State == eWorkerStateType.waiting then
...
function HandleLClick_WorkerMove_Button(workerIndex, idx)
	--_PA_LOG( "asdf", "HandleLClick_WorkerMove_Button  workerIndex : " .. workerIndex .. " idx : " .. tostring( idx ) .. "Worker_Table[workerIndex]._WorkerNo : " .. tostring( Worker_Table[workerIndex]._WorkerNo ) )
	local workerData = ToClient_getPlantWaitWorkerByIndex( plantKey, workerIndex)
	local workerStaticStatus = workerData:getWorkerStaticStatus()

	--local testWorker = ToClient_getNpcWorkingByWorkerNo( workerData:getWorkerNo():get_s64() )
...
function WorldMapWindow_OpenWaitWorkerManageByPlantKey( aPlantKey )
    plantKey = aPlantKey
	--[[
	local CheckControl = UI.getChildControl(Panel_Win_Worldmap, "Static_GuideBG")
	local PosX = CheckControl:GetPosX()
	local PosY = CheckControl:GetPosY()
	Panel_manageWorker:SetPosX(PosX)
	Panel_manageWorker:SetPosY(PosY)
	--]]
	scroll_Param._offsetIndex	= 0
	Control_RestoreItem_Reset()
	Worker_Table_Set()
	
--	Panel_manageWorker:SetShow(true);
end
...
--[[
function FromClient_WorkerDataUpdate_StopWorking(workerNo, isUserRequest)	
	for key, value in pairs (Worker_Table) do
		if nil ~= Worker_Table[key]._WorkerNo then
			if Worker_Table[key]._WorkerNo:get_s64() == workerNo then
				WorldMapWindow_WorkerManage_Refresh()
				return
			end
		end		
	end
...
-------------------------- 일꾼 Event 관련 함수 -----------------------------
-----------------------------------------------------------------------------
-- function FromClient_changeLeftWorking( workerNo )	-- 취소 예약 시 함수 실행.
-- 	local workStringNo = tostring(workerNo)
-- 	waitingCancelWorkerNo[workStringNo] = true
-- 	local self = Control_WorkerList
-- 	self:UpdateSlot(0, false)
-- 	-- self._worker_State[waitingCancelIdx]:SetEnable(false)
-- 	-- self._worker_State[waitingCancelIdx]:SetMonoTone(true)
-- 	-- self._worker_State[waitingCancelIdx]:SetText("취소예약")
-- end
...
function HandleOn_WorkerTooltip(idx)
	...
	if true == Worker_Table[workerIndex]._isEmpty then
		...
		-- local waitWorkerCount = ToClient_getPlantWaitWorkerListCount(plantKey, 0) -- 선행 함수가 어디선가 호출되나?
...
function HandleOut_WorkerTooltip(idx, isReset)
	...
	if WorkerTooltip_Index == idx then
		...
		if 0 < checkX and 0 < checkY and checkX < uiBase:GetSizeX() and checkY < uiBase:GetSizeY() and false == isReset then
			return
		end
...
function HandleClick_RestoreItem(idx)
	...
	requestRecoveryWorker(workerNo, slotNo)
end

...
-- Panel_Mail_Detail:SetShow(true)		-- 테스트용 삭제해야함
...
function Mail_Detail_HideAni()
	...
	local mailHideAni = Panel_Mail_Detail:addColorAnimation( 0.0, 0.22, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
...
local	mailDetail	= {
	...
	_buttonQuestion	= UI.getChildControl(Panel_Mail_Detail, "Button_Question"),		--물음표 버튼
...
function	mailDetail:registEventHandler()
	...
	self._buttonQuestion:addInputEvent( "Mouse_LUp",	"Panel_WebHelper_ShowToggle( \"PanelMailDetail\" )");		--물음표 좌클릭
	self._buttonQuestion:addInputEvent( "Mouse_On",	"HelpMessageQuestion_Show( \"PanelMailDetail\", \"true\")");			--물음표 마우스오버
	self._buttonQuestion:addInputEvent( "Mouse_Out",	"HelpMessageQuestion_Show( \"PanelMailDetail\", \"false\")");			--물음표 마우스아웃
...
function Mail_Detail_GetItem()
	...
	if toInt64(0,1) < giftCount_s64 then
		local getMygift = function ( count_s64 )
			RequestMail_receiveMailItem( count_s64 )
			return
		end
		Panel_NumberPad_Show( true, giftCount_s64, nil, getMygift, false, nil )
		-- Panel_NumberPad_Show( isShow, s64_maxNumber, param0, confirmFunction, isExchange, param1 )
	else
		RequestMail_receiveMailItem( toInt64(0,1) )
		return
	end
	-- RequestMail_receiveMailItem()
end

...
local	mailSend	= {	
	_buttonQuestion		= UI.getChildControl( Panel_Mail_Send,	"Button_Question"		),		--물음표 버튼
	_inventoryType		= 0,
	_inventorySlotNo	= -1,
}
...
function	MailSendHideAni()
	...
	local aniInfo1 = Panel_Mail_Send:addColorAnimation( 0.0, 0.22, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
...
function	mailSend:registEventHandler()	
	self._buttonQuestion:addInputEvent("Mouse_LUp",	"Panel_WebHelper_ShowToggle( \"PanelMailSend\" )"	)		--물음표 좌클릭
	self._buttonQuestion:addInputEvent("Mouse_On",	"HelpMessageQuestion_Show( \"PanelMailSend\", \"true\")"	)		--물음표 마우스오버
	self._buttonQuestion:addInputEvent("Mouse_Out",	"HelpMessageQuestion_Show( \"PanelMailSend\", \"false\")"	)		--물음표 마우스아웃
end
...
function MailSend_Open( inventoryType, inventorySlotNo )
	...
	UI.Set_ProcessorInputMode(IM.eProcessorInputMode_MailInputMode)
...
function MailSend_Close()
	...
	local aniInfo2 = Panel_Mail_Send:addScaleAnimation( 0.0, 0.22, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
...
--mailSend:init()
--mailSend:registEventHandler()
--mailSend:registMessageHandler()

...
function Panel_Villa_Auction_ShowAni()
	...
	local aniInfo1 = Panel_Villa_Auction:addScaleAnimation( 0.0, 0.08, UI_ANI_ADV.PAUI_ANIM_ADVANCE_COS_HALF_PI)
...
function Panel_Villa_Auction_HideAni()
	...
	local aniInfo1 = Panel_Villa_Auction:addColorAnimation( 0.0, 0.25, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
...
function VillaAuction_InitControl()
	...
	self.auctionHouseDescFrame	= {		
									[0] = {	Frame = UI.getChildControl( Panel_Villa_Auction,	"Frame_1_AuctionDesc" )	},
									[1] = {	Frame = UI.getChildControl( Panel_Villa_Auction,	"Frame_2_AuctionDesc" )	},
									[2] = {	Frame = UI.getChildControl( Panel_Villa_Auction,	"Frame_3_AuctionDesc" )	},
									[3] = {	Frame = UI.getChildControl( Panel_Villa_Auction,	"Frame_4_AuctionDesc" )	}
								}
...
function HandleClicked_VillaAuctionPageChange()
	...
	if (auctionType == 14) then
		RequestBiddingPage()
		VillaAuction.panelTitle:SetText( "내 빌라 입찰 목록" ) -- "길드 하우스 내 입찰 목록")
	elseif (auctionType == 5) then
		RequestAuctionListPage()
		VillaAuction.panelTitle:SetText( "빌라 경매" )
	end
end

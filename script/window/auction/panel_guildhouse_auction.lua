...
function Panel_GuildHouse_Auction_ShowAni()
	...
	local aniInfo1 = Panel_GuildHouse_Auction:addScaleAnimation( 0.0, 0.08, UI_ANI_ADV.PAUI_ANIM_ADVANCE_COS_HALF_PI)
...
function Panel_GuildHouse_Auction_HideAni()
	...
	local aniInfo1 = Panel_GuildHouse_Auction:addColorAnimation( 0.0, 0.25, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
...
function GuildHouseAuction_InitControl()
	...
	self.btn_Question:addInputEvent( "Mouse_LUp", "Panel_WebHelper_ShowToggle( \"HouseAuction\" )" )				-- 물음표 좌클릭
	self.btn_Question:addInputEvent( "Mouse_On", "HelpMessageQuestion_Show( \"HouseAuction\", \"true\")" )			-- 물음표 마우스오버
	self.btn_Question:addInputEvent( "Mouse_Out", "HelpMessageQuestion_Show( \"HouseAuction\", \"false\")" )		-- 물음표 마우스아웃
...
function HandleClicked_GuildHouseAuctionPageChange()
	...
	if (auctionType == 1) then
		RequestBiddingPage()
		GuildHouseAuction.panelTitle:SetText( PAGetString(Defines.StringSheet_GAME, "LUA_GUILDHOUSE_AUCTION_MYAUCTION") ) -- "길드 하우스 내 입찰 목록")
	elseif (auctionType == 5) then
		RequestAuctionListPage()
		GuildHouseAuction.panelTitle:SetText( PAGetString(Defines.StringSheet_GAME, "LUA_GUILDHOUSE_AUCTION_AUCTIONLIST") ) -- "길드 하우스 경매")
	end
end
...
function GuildHouseAuctionManager:initialize()
	for y=0, (maxYCount - 1), 1 do
		for x=0, (maxXCount - 1), 1 do
...
function HandleClickedAuctionBidGuildHouse(index)
	...
	if nil == editPrice[index] then
		Proc_ShowMessage_Ack( PAGetString(Defines.StringSheet_GAME, "LUA_GUILDHOUSE_AUCTION_WRONGPRICE") )	-- 	"입찰 금액이 잘못되어 있습니다."
		return
	end

	local price					= Uint64toUint32(editPrice[index])
	local myAuctionInfo			= RequestGetAuctionInfo()
	...
	if ( Uint64toUint32(houseAuctionNowPrice:getPrice_s64()) <= price ) then
		ClearFocusEdit()
...
function GuildHouseAuctionManager:setHouseData(index)
	...
	--	self._houseAuctionList[index]._aucPrice:SetText( (PAGetString(Defines.StringSheet_GAME, "LUA_HOUSEAUCTION_NOWPRICE")) .." ".. tostring(Uint64toUint32(houseAuctionInfo:getHighPrice_s64())) .. " " .. (PAGetString(Defines.StringSheet_GAME, "LUA_AUCTION_GOLDTEXT")))
	--else		
...
function GuildHouseAuctionManager:setBidData(index)
	...
	--[[
	local characterStaticWarpper =  bidAuctionInfo:getCharacterStaticStatus()	
	local screenShotPath 	= characterStaticWarpper:getObjectStaticStatus():getHouseScreenShotPath(0)	-- 스크린샷 ( 0 ~ 3번까지 가져올 수 있다 )
	local descArea 			= characterStaticWarpper:getObjectStaticStatus():getDescArea()				-- 면적
	local descFeature1		= characterStaticWarpper:getObjectStaticStatus():getDescFeature1()			-- 기본 제공
	local descFeature2		= characterStaticWarpper:getObjectStaticStatus():getDescFeature2()			-- 특징
	]]--
	
	if( bidAuctionInfo:isAuctionEnd() ) then
		self._houseAuctionList[index]._btn_Cancel_Get:SetShow(true)
		self._houseAuctionList[index]._btn_Cancel_Get:SetText( PAGetString(Defines.StringSheet_GAME, "LUA_HOUSEAUCTION_RETURNMONEY") ) -- 입찰금 회수
	else
	...
	self._houseAuctionList[index]._remainCount:SetText( PAGetStringParam1( Defines.StringSheet_GAME, "LUA_GUILDHOUSE_AUCTION_REMAININGCOUNT", "getRemainBidCount", tostring(bidAuctionInfo:getRemainBidCount()) ) ) -- "남은 입찰 횟수 : " ..tostring(bidAuctionInfo:getRemainBidCount()) );

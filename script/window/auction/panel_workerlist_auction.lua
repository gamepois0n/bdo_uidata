...
function MyworkerList:Init()
	for ii=0, self._slotMaxCount - 1 do
		...
		slot._btnResist		:SetPosY( slot._workerName:GetPosY() + 7 )
...
local MyworkerList_Update = function()
	...
	for worker_Index = self._startIndex, self._listCount - 1 do
		...
		if nil ~= workerWrapperLua then			
			...
			if true == workerWrapperLua:getIsAuctionInsert() then									-- 일꾼 거래소에 등록 중이다.
				slot._btnResist	:SetEnable( false )
				slot._btnResist	:SetMonoTone( true )
				slot._btnResist	:SetFontColor( UI_color.C_FF515151 )
				slot._btnResist	:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_WORKERAUCTION_REGISTINGNOW")) -- "등록중" 
			elseif true == workerWrapperLua:isWorking() then										-- 작업 중이다.
				slot._btnResist	:SetEnable( false )
				slot._btnResist	:SetMonoTone( true )
				slot._btnResist	:SetFontColor( UI_color.C_FF515151 )
				slot._btnResist	:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_WORKERAUCTION_WORKINGNOW")) --  "작업중" 
			elseif workerWrapperLua:getActionPoint() ~= workerWrapperLua:getMaxActionPoint() then	-- 행동력이 채워져있지 않다.
				slot._btnResist	:SetEnable( false )
				slot._btnResist	:SetMonoTone( true )
				slot._btnResist	:SetFontColor( UI_color.C_FF515151 )
				slot._btnResist	:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_WORKERAUCTION_NOTENOUGH_ACTIONPOINT")) -- "행동력\n부족" 
			elseif workerWrapperLua:getGrade() < 2 then
				slot._btnResist	:SetEnable( false )
				slot._btnResist	:SetMonoTone( true )
				slot._btnResist	:SetFontColor( UI_color.C_FF515151 )
				slot._btnResist	:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_WORKERAUCTION_NOTENOUGH_GRADE")) -- "등급\n미달" 
			elseif workerWrapperLua:getWorkerDefaultSkillStaticStatus() then
				slot._btnResist	:SetEnable( false )
				slot._btnResist	:SetMonoTone( true )
				slot._btnResist	:SetFontColor( UI_color.C_FF515151 )
				slot._btnResist	:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_WORKERAUCTION_CANNOT_REGIST")) -- "불가" 
			else
				slot._btnResist	:SetEnable( true )
				slot._btnResist	:SetMonoTone( false )
				slot._btnResist	:SetFontColor( UI_color.C_FFEFEFEF )
				slot._btnResist	:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_WORKERAUCTION_REGIST")) -- "등록"
			end	
			
			slot._workerIcon			: ChangeTextureInfoName( workerIcon )
			slot._workerLv				: SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_WORKERAUCTION_WORKERLEVEL", "level", tostring(workerLv) )) -- "Lv." .. tostring(workerLv)
			...
			slot._upgradeChance			: SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_WORKERAUCTION_UPGRADECOUNT", "upgradecount", workerUpgradeCount)) --  "승급 기회: " .. workerUpgradeCount 
			...
			if 4 <= workerWrapperLua:getGrade() then -- 장인 일꾼은 승급 불가 표시.
...
function FGlobal_AuctionResist_WorkerList ()
	...
	if 0 < self._listCount then	
		...
		Proc_ShowMessage_Ack( PAGetString(Defines.StringSheet_GAME, "LUA_WORKERAUCTION_NOWORKER") ) -- "해당 지역에 등록할 수 있는 일꾼이 없습니다."
		return
	end
end

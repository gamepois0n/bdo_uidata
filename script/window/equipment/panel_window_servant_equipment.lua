...
function ServantEquipment_MouseOn( slotNo, isOn )
	Panel_Tooltip_Item_Show_GeneralNormal(slotNo, "servant_inventory", isOn)
end
...
function ServantEquipment_LClick( slotNo )
	if DragManager.dragStartPanel == Panel_Window_Inventory then
		...
		--if nil ~= itemWrapper then
		--  servantEquipmentDoUnequip( slotNo )
	  --end
	end
end
...
function ServantEquipment_HideAni()
	...
	local aniInfo1 = Panel_ServantEquipment:addColorAnimation( 0.0, 0.22, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
	...
	local aniInfo2 = Panel_ServantEquipment:addScaleAnimation( 0.0, 0.22, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)

...
function CashRevivalBuy:Update()
	...
	for itemIdx = 0, CashRevivalDataCount - 1 do
		...
		if( nil == CashRevivalData[itemIdx].expirationDate) then
			...
			--"기간 : "..tostring(CashRevivalData[itemIdx].expirationDate).."까지"
...
function HandleClicked_Buy_CashRevivalItem( respawnType )
	_selectSpawnType = respawnType
	local self 			= CashRevivalBuy
	local msgTitle		= PAGetString(Defines.StringSheet_GAME, "EXCHANGE_TEXT_STATE_FINAL_CHECK") -- "확인"
	local msgContent	= ""
	
	local cPSSW		= ToClient_GetCashProductStaticStatusWrapperByKeyRaw( getRevivalItem() )
	--msgContent	= "[" .. tostring(cPSSW:getName()) .. "( 가격 : ".. tostring(cPSSW:getPrice()) .."펄 )]\n이 아이템을 구매하시겠습니까?" .. "\n(로그아웃 후 접속한 상태면 경험치가 복구되지 않습니다.)"
...
function ToClient_Buy_CashRevivalBuy_Do()
	local cPSSW		= ToClient_GetCashProductStaticStatusWrapperByKeyRaw( getRevivalItem() )
	getIngameCashMall():requestBuyItem(cPSSW:getNoRaw(), 1, cPSSW:getPrice(), CppEnums.BuyItemReqTrType.eBuyItemReqTrType_ImmediatelyUse)
end
...
function HandleClicked_Apply_CashRevivalItem( respawnType )
	...
	--msgContent	= "[" .. CashRevivalData[self.SelectedItemNo].name .. "]\n이 아이템을 적용하시겠습니까?" .. "\n(로그아웃 후 접속한 상태면 경험치가 복구되지 않습니다.)"
...
function ToClient_CashRevivalBuy_Confirm_Do()
	...
	_PA_LOG( "Revival", "_selectSpawnType : " .. tostring( _selectSpawnType ) )
	deadMessage_Revival( _selectSpawnType, CashRevivalData[self.SelectedItemNo].slotNo, CashRevivalData[self.SelectedItemNo].itemWhereType, getSelfPlayer():getRegionKey() )
	-- if enRespawnType.respawnType_Immediate == _selectSpawnType then
		-- deadMessage_Immediate(CashRevivalData[self.SelectedItemNo].slotNo, CashRevivalData[self.SelectedItemNo].itemWhereType)
	-- elseif enRespawnType.respawnType_Town == _selectSpawnType then
		-- deadMessage_MoveToVillage( CashRevivalData[self.SelectedItemNo].slotNo, CashRevivalData[self.SelectedItemNo].itemWhereType )
	-- elseif enRespawnType.respawnType_Exploration == _selectSpawnType then
		-- deadMessage_MoveToNearExploration( CashRevivalData[self.SelectedItemNo].slotNo, CashRevivalData[self.SelectedItemNo].itemWhereType )
	-- else
		-- PA_ASSERT( false, "비정상적인 부활 타입이다." )
	-- end

...
function PearlCharge:update()
	...
	local productCount = getIngameCashMall():getCashProductListCountByCategory(CppEnums.CashProductCategory.eCashProductCategory_Pearl)
	local product = nil
...
function HandleClicked_PearlCharge_SelectedProduct( productIndex )
	...
	local product = getIngameCashMall():getCashProductStaticStatusByCategory( CppEnums.CashProductCategory.eCashProductCategory_Pearl, productIndex )
...
function HandleClicked_PearlCharge_Confirm()
	...
	local msgTitle			= PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYPEARLBOX_CONFIRM") -- "확인"
	local addPearlComment 	= PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_CONFIRM_PURCHASELIMIT")
	local msgContent		=  PAGetStringParam1( Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYPEARLBOX_BUYCONFIRM", "getName", cashProduct:getName() ) -- "[" .. cashProduct:getName() .. "]\n이 아이템을 구매하시겠습니까?"
...
function _IngameCashShop_PearlCharge_ConfirmDO()
	...
	getIngameCashMall():requestBuyItem( self.selected_productNoRaw, 1, cashProduct:getCashPrice(), CppEnums.BuyItemReqTrType.eBuyItemReqTrType_UiUpdate)
end

function FGlobal_IngameCashShop_PearlCharge_Open()
	local msgTitle			= PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYPEARLBOX_CONFIRM") -- "확인"
	local msgContent		= PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYPEARLBOX_PEARLCHARGE_MSG") -- "펄 상점에서 펄을 구입 후 이용해 주시기 바랍니다."
	...
	MessageBox.showMessageBox(messageboxData)
	--PearlCharge:update()
	--PearlCharge:SetPosition()
	--PearlCharge:SetWindowSize()
	--Panel_IngameCashShop_BuyPearlBox:SetShow( true )
end

...
function FromClient_RenderStateInformationChange( state )
	...
	if( WORLDMAP_RENDERTYPE.eWMS_EXPLORE_PLANT == state ) then
		...
		infoDesc		:SetText( PAGetString(Defines.StringSheet_GAME, "LUA_NEW_WORLDMAP_INFOMATION_INFODESC") ) -- "탐험한 지역과 거점 및 의뢰 그리고 지식 정보가 표시됩니다." )
		infoDesc		:SetSize( infoDesc:GetSizeX(), infoDesc:GetSizeY() )
		infoDescBG		:SetSize( infoDescBG:GetSizeX(), infoDesc:GetSizeY() + 10 )
		gapY = infoDescBG:GetSpanSize().y + infoDescBG:GetSizeY() + 5
		
		infoCity 		:SetSpanSize( infoCity:GetSpanSize().x, gapY )
		infoVillage 	:SetSpanSize( infoVillage:GetSpanSize().x, gapY )
		infoGate 		:SetSpanSize( infoGate:GetSpanSize().x, gapY + 24)
		infoTrade 		:SetSpanSize( infoTrade:GetSpanSize().x, gapY + 24)
		infoDangerous 	:SetSpanSize( infoDangerous:GetSpanSize().x, gapY + 48)
		infoEmpty 		:SetSpanSize( infoEmpty:GetSpanSize().x, gapY + 48)		
		
		gapY = infoEmpty:GetSpanSize().y + infoEmpty:GetSizeY()
		Panel_Worldmap_Information:SetSize( Panel_Worldmap_Information:GetSizeX(), gapY + 10 )			
	...
	elseif( WORLDMAP_RENDERTYPE.eWMS_REGION == state ) then
		...
		infoDesc:SetSize( infoDesc:GetSizeX(), infoDesc:GetSizeY() )
		infoDescBG:SetSize( infoDescBG:GetSizeX(), infoDesc:GetSizeY() + 7 )

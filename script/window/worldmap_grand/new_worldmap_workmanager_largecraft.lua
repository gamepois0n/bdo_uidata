...
function Panel_LargeCraft_WorkManager_ShowAni()
	local aniInfo1 = Panel_LargeCraft_WorkManager:addScaleAnimation( 0.0, 0.08, UI_ANI_ADV.PAUI_ANIM_ADVANCE_COS_HALF_PI)
...
function Panel_LargeCraft_WorkManager_HideAni()
	...
	local aniInfo1 = Panel_LargeCraft_WorkManager:addColorAnimation( 0.0, 0.15, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
...
defalut_Control._subWork_Info._immediately:SetShow(false);	-- 상용화전에는 나오면 안됩니다 -- 임시처리(who1310)
...
-----------------------------------------------------
-- 대규모 작업 정보(LargCraft Info) 관련 함수
-----------------------------------------------------
...
function HandleClick_LargCraft_List_Btn(isLeft)
	if isLeft and 1 < LargCraft_List._currentIndex then
		...
		defalut_Control._subWork_Estimated._Time_Count:SetText( PAGetStringParam1( Defines.StringSheet_GAME, "LUA_NEW_WORLDMAP_WORKMANAGER_BUILDING_DEFAULT", "getWorkingCount", Subwork_Progress_List_GetWorkingCount() ) ) -- "(" .. Subwork_Progress_List_GetWorkingCount() .. "회)" )
...
function LargCraft_Cancel_All()
	...
	if workingCout > 0 then		
		_Cancel_All = true -- 일꾼 작업 중지 Event에서 대규모 제작 취소를 처리한다.
		...
		_Cancel_All = false	-- 취소할 일꾼 작업이 없으면 바로 대규모 제작 취소를 처리한다.
...
function HandleClick_LargCraft_Cancel_Btn()
	...
	local messageboxData = { title = PAGetString(Defines.StringSheet_GAME, "LUA_LARGECRAFT_WORKMANAGER_CANCELALL_TITLE" ), content = cancelContent , functionYes = LargCraft_Cancel_All, functionCancel = MessageBox_Empty_function, priority = UI_PP.PAUIMB_PRIORITY_LOW }
	MessageBox.showMessageBox(messageboxData, "top")
	
end

-----------------------------------------------------
-- 작업 목록(SubWork List) 관련 함수
-----------------------------------------------------
	...
	for index = 1, LargCraft_List._contentCount do	
		...
		if esSSW:isSet() then
			...
			for idx = 1, eSSCount do	
				...
				-- { 이미 진행중인 작업이 있을 때 꼬이는 문제 해결용.
					local workingCount			= ToClient_getHouseWorkingWorkerList( houseInfoSS )	-- 이미 진행중인 작업이 있을 때 꼬이는 문제 해결용.
					local tempCount				= 0
				...
				self._data_Table[index]._resource[idx] = 
				{
					...
					_workingCount	= tempCount,	-- 이미 진행중인 작업이 있을 때 꼬이는 문제 해결용.
				}
...
function SubWork_List:_setOffset()
	...
	if esSSW:isSet() then
		...
		UIScroll.SetButtonSize( defalut_Control._subWork_List._Scroll, self._rowMax, self._contentRow ) -- 스크롤바 버튼의 크기를 조정한다.
...
function HandleOn_SubWork_List(index)
	...
	if SubWork_List._selected_Index ~= SubWork_List._over_Index then
		...
		if data ~= nil then
			...
			if _onGoingCount > 0 then					
				-- defalut_Control._subWork_List._Progress_OnGoing[index]	:SetShow(true)
...
function SubWork_List_Immediately_CraftItem()	-- 아이템 제작 즉시 완료
	...
	if esSSW:isSet() then
		...
		local messageBoxMemo	= PAGetStringParam2( Defines.StringSheet_GAME, "LUA_NEW_WORLDMAP_WORKMANAGER_HOUSE_IMMEDIATELY", "workName", workName, "needPearl", tostring(defalut_Control._subWork_info._needPearl) ) -- "[ ".. workName .. " ]\n" .. tostring(defalut_Control._subWork_info._needPearl) .. " 펄을 사용해서 즉시 완료 하시겠습니까?"
		local messageboxData = { title = messageboxTitle, content = messageBoxMemo , functionYes = SubWork_List_Immediately_CraftItemYes, functionCancel = MessageBox_Empty_function, priority = UI_PP.PAUIMB_PRIORITY_LOW }
	...
	if esSSW:isSet() then
		...
		for Index = 1, waitingWorkerCount do
			...
			if( true == ToClient_isWaitWorker( npcWaitingWorker ) and ( false == workerWrapperLua:getIsAuctionInsert() ) ) then
				...
				local workSpeed				= npcWaitingWorker:getWorkEfficienceWithSkill(checkData, productCategory)	-- 대량제작
				...
				if ( isWorkable ) then
					...
					name				= "Lv."..npcWaitingWorker:getLevel() .. " " .. PAGetStringParam2( Defines.StringSheet_GAME, "LUA_NEW_WORLDMAP_WORKMANAGER_HOUSE_NOTFINDNODE", "npcWaitingWorkerSS", getWorkerName(npcWaitingWorkerSS), "getAreaName", workerRegionWrapper:getAreaName() ) -- getWorkerName(npcWaitingWorkerSS) .. "(<PAColor0xff868686>" .. workerRegionWrapper:getAreaName() .. "-<PAOldColor><PAColor0xffFF0000>연결안됨<PAOldColor>)"
				end
...
function Subwork_Progress_List_UpdateSlot(isRefresh, isOver)
	...
	if true == isRefresh then
		...
		if 0 == Subwork_Progress_List._onGoingCount then
			...
			-- if(FGlobal_IsCommercialService()) then									-- 상용화 기간 체크
	...
	for idx = 1, Subwork_Progress_List._rowMax do
		...
		if 0 < ToClient_getNpcWorkerWorkingCount( workerNo ) then
			defalut_Control._subWork_Info._OnGoing_Cancel[idx]:SetText( PAGetStringParam1( Defines.StringSheet_GAME, "LUA_NEW_WORLDMAP_WORKMANAGER_BUILDING_ONGOING", "workerNo", ToClient_getNpcWorkerWorkingCount(workerNo) ) ) -- "(" .. ToClient_getNpcWorkerWorkingCount( workerNo ) .. "회 남음) ")
		else
...
--	Panel_LargeCraft_WorkManager:SetShow(false)
	FGlobal_WorldMapWindowEscape();
end
...
function HandleClick_doWork_LargeCraft()
	...
	if ( currentKey ~= selected_Work ) then
		...
		local messageboxData = { title = PAGetString(Defines.StringSheet_GAME, "LUA_LARGECRAFT_WORKMANAGER_STARTWORK_TITLE" ), content = _content , functionYes = HandleClick_doWork_LargeCraft_Continue, functionCancel = MessageBox_Empty_function, priority = UI_PP.PAUIMB_PRIORITY_LOW }
		MessageBox.showMessageBox(messageboxData, "top")		
	else
		if 0 < LimitWorkableCount_LargeCraft() then
			...
			if 0 < workerActionPoint then
				Proc_ShowMessage_Ack( PAGetString(Defines.StringSheet_GAME, "LUA_WORLDMAP_WORKERMANAGER_LARGECRAFT_MAXWORKCOUNT" ) )	-- "진행 중인 작업수가 이미 최대입니다."
			else
...
function LimitWorkableCount_LargeCraft()
	...
	local totalWorkCount	= SubWork_List._data_Table[workIndex]._resource[subWorkIndex]._fullCount		-- 토탈 카운트
	local currentWorkCount	= SubWork_List._data_Table[workIndex]._resource[subWorkIndex]._progressCount	-- 현재 카운트
	local workingCount		= SubWork_List._data_Table[workIndex]._resource[subWorkIndex]._workingCount 	-- 진행중인 카운트
	local workableCount		= totalWorkCount - currentWorkCount	- workingCount								-- 일 할 수 있는 카운트
	local resourceCount		= SubWork_List._data_Table[workIndex]._resource[subWorkIndex]._haveCount		-- 재료 수
	local workerActionPoint	= Worker_List._data_Table[Worker_List._selected_Index]._currentPoint			-- 현재 행동력
	local ongoingCount		= SubWork_List._data_Table[workIndex]._onGoingCount								-- 해당 작업에 붙어 있는 일꾼수(작업중인 일 수)
...
function Set_Workable_Count_LargetCraft( inputNumber )
	...
	defalut_Control._subWork_Estimated._Time_Count:SetText( PAGetStringParam1( Defines.StringSheet_GAME, "LUA_NEW_WORLDMAP_WORKMANAGER_BUILDING_DEFAULT", "getWorkingCount", Subwork_Progress_List._workingCount ) ) -- "(" .. Subwork_Progress_List._workingCount .. "회)" )
end
...
function HandleClick_OnGoing_Cancel_LargeCraft(idx)
	...
	if _leftWorkCount < 1 then
		...
		local messageboxData = { title = PAGetString(Defines.StringSheet_GAME, "LUA_LARGECRAFT_WORKMANAGER_CANCELWORK_TITLE" ), content = cancelWorkContent , functionYes = HandleClick_OnGoing_Cancel_LargeCraft_Continue, functionCancel = MessageBox_Empty_function, priority = UI_PP.PAUIMB_PRIORITY_LOW }
		MessageBox.showMessageBox(messageboxData, "top")
	end
end

function HandleClick_OnGoing_Cancel_LargeCraft_Continue()
	-- local isSuccess = ToClient_requestStopPlantWorking(worker_StopWork)
...
function FromClient_LargeCraft_WorkManager_StopWorkerWorking(workerNo, isUserRequest)
	if true == _Cancel_All then	-- 대규모 제작 취소에 일꾼 작업 취소가 필요한 경우, 여기에서 제작 취소를 처리한다.
		local workingCout			= ToClient_getHouseWorkingWorkerList( houseInfoSS )
	...
	-- for key, value in pairs (LargCraft_List._workerList) do
...
function FGlobal_LargeCraft_WorkManager_Refresh( dontUpdateWorker )
	if Panel_LargeCraft_WorkManager:GetShow() then
		...
		-- LargCraft_List:_updateSlot()에서 SubWork_List:_setData()때 셋팅되는 데이터를 쓰기때문에 주의해야함.
		LargCraft_List:_updateSlot()
		SubWork_List:_updateSlot()
		if ( false == dontUpdateWorker ) then
			Worker_List:_updateSlot()

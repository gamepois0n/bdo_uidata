...
function HouseInstallationCart:Update( nowInterval )
	...
	self.have_PearlValue:SetText( makeDotMoney(s64_havePearls) ); -- 가진돈
	self.sum_PearlValue:SetText( "0" )--장바구니돈
...
function HandleClicked_InstallationModeCart_BuyItemAll()
	...
	if getSelfPlayer():get():isMyHouseVisiting() then
		messageBoxMemo = PAGetString(Defines.StringSheet_GAME, "LUA_HOUSE_INSTALLATIONMODE_CART_BUYITEMALL_MSGMEMO1") -- "장바구니에 담긴 가구를 모두 구입하시겠습니까?"
	else
		messageBoxMemo = PAGetString(Defines.StringSheet_GAME, "LUA_HOUSE_INSTALLATIONMODE_CART_BUYITEMALL_MSGMEMO2") -- "장바구니에 담긴 <PAColor0xffddcd82>가구를 선물<PAOldColor>하시겠습니까?\n선물된 가구는 회수할 수 없습니다."
	end

...
function HandleClicked_WorkerChangeSkill_Do()
	...
	if nil == prevWorkerSkillKeyRaw or nil == workerNoRaw then
		Proc_ShowMessage_Ack( PAGetString(Defines.StringSheet_GAME, "LUA_WORKERCHANGESKILL_NOSELECTSKILL") ) -- 일꾼 혹은 스킬이 선택되지 않았습니다.
		return
	end
...
function FromClient_ChangeWorkerSkillNoOne( workerNoRaw )
	...
	Proc_ShowMessage_Ack( PAGetStringParam1( Defines.StringSheet_GAME, "LUA_WORKERCHANGESKILL_WORKERCHANGESKILL", "name", name ) ) -- name .. "의 스킬이 변경됐습니다." )
end

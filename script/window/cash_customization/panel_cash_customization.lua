...
local TemplateUI			= {
	...
	-- BTN_Buy				= UI.getChildControl( Panel_Cash_Customization, "Template_Button_Buy"),
}
...
function CashCustomization:Update()
	...
	local productSize = getIngameCashMall():getCashProductListCountByCategory(CppEnums.CashProductCategory.eCashProductCategory_Customize)
	local product = nil
	for productIdx = 0, productSize -1 do
		product = getIngameCashMall():getCashProductStaticStatusByCategory(CppEnums.CashProductCategory.eCashProductCategory_Customize, productIdx)
		if (product ~= nil) then
	...
	for cumaIdx = 0, CashCustomizationDataCount - 1 do
		if nil == self.ItemUiPOOL[cumaIdx] then
			...
			-- CreateItemName:SetText( self.SetCharacterEquipName[cumaIdx] )
			...
			-- CreateItemPeriod:SetText( self.SetCharacterEquipName[cumaIdx] )
		...
		if (CashCustomizationData[cumaIdx] ~= nil) and (Defines.s64_const.s64_0 < CashCustomizationData[cumaIdx].count) then
			...
			if(CashCustomizationData[cumaIdx].count == 0) then
				...
				UiBase.ItemPeriod:SetText( PAGetString(Defines.StringSheet_GAME, "LUA_CASH_CUSTOMIZATION_ITEMPERIOD2") ) -- 이용권이 없습니다.
			else
...
function CashCumaBuy:Update(applyType)
	...
	elseif(applyType == "pearl") then 
		local productSize = getIngameCashMall():getCashProductListCountByCategory(CppEnums.CashProductCategory.eCashProductCategory_Pearl)
		local product = nil
		for productIdx = 0, productSize -1 do
			product = getIngameCashMall():getCashProductStaticStatusByCategory(CppEnums.CashProductCategory.eCashProductCategory_Pearl, productIdx)
			if (product ~= nil) then
				CashCustomizationData[idx] 				= {}
				CashCustomizationData[idx].name 		= product:getName()
				CashCustomizationData[idx].productNo 	= product:getNoRaw()
	...
	if(applyType == "inven") then 
		self.staticText_notify:SetText( PAGetString(Defines.StringSheet_GAME, "LUA_CASH_CUSTOMIZATION_STATICTEXT_NOTIFY1") ) -- 사용할 수 있는 아이템이 있습니다.\n아래 리스트에서 사용할 아이템을 선택해주세요.
	else
		self.staticText_notify:SetText( PAGetString(Defines.StringSheet_GAME, "LUA_CASH_CUSTOMIZATION_STATICTEXT_NOTIFY2") ) -- 아래 리스트에서 구매할 아이템을 선택해주세요.
	end
...
function _ingameCustomization_BuffTooltip( isShow )
	...
	if true == isShow then
		...
		local name						= PAGetString(Defines.StringSheet_GAME, "LUA_CASH_CUSTOMIZATION_BUFFTOOLTIP_NAME") -- "외형 변경권(30일)"
		local desc						= PAGetStringParam1( Defines.StringSheet_GAME, "LUA_CASH_CUSTOMIZATION_BUFFTOOLTIP_DESC", "customizationPackageTime", convertStringFromDatetime( toInt64( 0, customizationPackageTime ) ) ) -- "사용 기간이 " .. convertStringFromDatetime( toInt64( 0, customizationPackageTime ) ) .. " 남았습니다."
...
function CashCustomization_Open()
	...
	UIAni.AlphaAnimation( 1, Panel_Cash_Customization, 0.0, 0.2 )
end
...
function ToClient_CashCumaBuy_Confirm_Do()
	...
	elseif(CashCumaBuy.ApplyType == "goods") then
		getIngameCashMall():requestBuyItem(CashCustomizationData[self.SelectedItemNo].productNo, 1, CashCustomizationData[self.SelectedItemNo].pearlPrice, CppEnums.BuyItemReqTrType.eBuyItemReqTrType_UiUpdate)
	elseif(CashCumaBuy.ApplyType == "pearl") then
		getIngameCashMall():requestBuyItem(CashCustomizationData[self.SelectedItemNo].productNo, 1, CashCustomizationData[self.SelectedItemNo].cashPrice, CppEnums.BuyItemReqTrType.eBuyItemReqTrType_UiUpdate)
	else	
		getIngameCashMall():requestBuyItem(CashCustomizationData[self.SelectedItemNo].productNo, 1, CashCustomizationData[self.SelectedItemNo].pearlPrice, CppEnums.BuyItemReqTrType.eBuyItemReqTrType_ImmediatelyUse)
	end
end
...
function CashCustomization:registEventHandler()
	...
	-- self.BTN_BuyPearl	:addInputEvent( "Mouse_LUp", "CashCumaBuy_Open('pearl')" ) 
end

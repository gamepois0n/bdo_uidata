...
function ExtendExpiration:Close()
	...
	Equipment_SetShow(true)	-- 꺼진 장비창을 켜준다.
end
...
function ExtendExpiration_Inventory_Rclick( slotNo, itemWrapper, count_s64, inventoryType )
	...
	if true == isOverExtendedExpiration then
		local	messageBoxMemo = PAGetString ( Defines.StringSheet_GAME, "LUA_EXTENDEXPIRATION_DONT_OVERPERIOD")	-- "이 아이템은 원래 기간을 초과할 수 없습니다. \n기간을 연장하시겠습니까?"
...
function HandleClicked_ExtendExpiration_ChangeMaterialCount( isPlus )
	if true ~= ExtendExpiration.config.isSetItem then
		Proc_ShowMessage_Ack( PAGetString(Defines.StringSheet_GAME, "LUA_EXTENDEXPIRATION_SELECTITEM") ) -- "대상 아이템을 선택해야 합니다."
		return
	end
	...
	if isPlus then
		if maxCount <= nowCount then
			Proc_ShowMessage_Ack( PAGetString(Defines.StringSheet_GAME, "LUA_EXTENDEXPIRATION_EXPIRATION_MAX") ) -- "연장할 수 있는 최대 갯수입니다."
			return
		end
		ExtendExpiration.config.materialCount = ExtendExpiration.config.materialCount + toInt64(0,1)
	else
		if nowCount <= toInt64(0,1) then
			Proc_ShowMessage_Ack( PAGetString(Defines.StringSheet_GAME, "LUA_EXTENDEXPIRATION_EXPIRATION_MIN") ) -- "최소 갯수입니다."
			return
		end
		ExtendExpiration.config.materialCount = ExtendExpiration.config.materialCount - toInt64(0,1)
	end
...
function FromClient_SucceedExtendedExpirationPeriod(targetWhereType, targetSlotNo)	-- 기간연장에 성공했다.
	...
	if nil ~= targetItemWrapper then
		...
		Proc_ShowMessage_Ack( PAGetStringParam1(Defines.StringSheet_GAME, "LUA_EXTENDEXPIRATION_SUCCEEDEXTENDPERIOD", "name", targetItemName) )	-- targetItemName .. " 기간 연장이 완료됐습니다." )
	end
end

...
function HandleClicked_DyeNew_ActionChange(index)
	ToClient_setActionChart( index + 2 )
end
...
function HandleClicked_DeyNew_RandomPose()
	-- 랜덤액션
	local randValue = math.random(3, 8)
...
-- local weatherTable = ToClient_RequestUpdateDyeGetCustomWeatherTable()
-- local weatherTime = ToClient_RequestUpdateDyeGetCustomWeatherTime()
end

function HandleOver_DyeNew_SimpleTooltips( isShow, tipType )
	local name, desc, control = nil, nil, nil
	if 0 == tipType then		-- 동작 취하기.
		name	= PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_SETEQUIP_POSE") -- "동작 취하기"

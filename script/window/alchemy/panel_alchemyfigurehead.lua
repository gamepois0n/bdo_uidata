...
local totemDesc = PAGetString(Defines.StringSheet_GAME, "LUA_ALCHEMYFISHPRINT_DESC")	-- "※ 바다의 토템은 이동속도 등 특별한 효과를 얻는 선박 전용 장비 아이템으로, 낚시 상인에게 구입할 수 있습니다.\n※ 토템과 이름이 같은 물고기를 낚으면 경험치가 오릅니다.\n※ 경험치 80%부터 블랙스톤(무기)로 성장시킬 수 있습니다.\n※ 단, 성장 실패 시 경험치나 단계가 떨어질 수 있습니다."
...
function AlchemyFigureHead:Init()
	...
	-- 컨텐츠 BG 텍스처 지정
	self.control.contentBG		:ChangeTextureInfoName( AlchemyFigureHeadTabTexture[2].bg )	-- 텍스쳐 변경
	-- self.control.contentEffect	:ChangeTextureInfoName( AlchemyFigureHeadTabTexture[2].effect )	-- 텍스쳐 변경
...
function AlchemyFigureHead:registMessageHandler()
	...
	registerEvent( "FromClient_StoneChange", 				"FromClient_FigureHeadChange" )					-- 성장 성공
	registerEvent( "FromClient_StoneChangeFailedByDown", 	"FromClient_FigureHeadChangeFailedByDown" )		-- 성장 실패
	registerEvent( "FromClient_AlchemyEvolve", 				"FromClient_FigureHeadEvolve" )				-- 성장 결과 전달용
...
function AlchemyFigureHead:ClearData( isOpenStep )
	...
	self.FigureHead_slot		:clearItem()	-- 슬롯을 초기화 한다.
	self.Stuff_slot.Empty 		= true
	self.FigureHead_slot.Empty 	= true
	...
	local guideKeyword = PAGetString(Defines.StringSheet_GAME, "LUA_ALCHEMYSTONE_UPGRADE")	-- "성장"
	local guideText = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_ALCHEMYFISHPRINT_SELECT", "guideKeyword", guideKeyword)	-- "가방에서 {guideKeyword}시킬 '바다의 토템'을 선택해주세요."
...
function HandleClicked_AlchemyFigureHeadTab( tabIdx )
	...
	AlchemyFigureHead.FigureHead_slot			:clearItem()	-- 슬롯을 초기화 한다.
	AlchemyFigureHead.Stuff_slot				:clearItem()	-- 슬롯을 초기화 한다.
	...
	AlchemyFigureHead.control.contentBG		:ChangeTextureInfoName( AlchemyFigureHeadTabTexture[tabIdx].bg )		-- 텍스쳐 변경

	AlchemyFigureHead.selectedTabIdx		= tabIdx

	AlchemyFigureHead.isPushDoit			= false	-- 탭 버튼 누르면, 진행 중인 작업이 취소.
...
function HandleClicked_AlchemyFigureHead_Doit()
	local doUpgrade = function()
		AlchemyFigureHead.isPushDoit	= true											-- 시작 여부 변수 적용
	end
	...
	local msgTitle				= PAGetString(Defines.StringSheet_GAME, "LUA_ALERT_DEFAULT_TITLE")	-- "알 림"
	local msgContent			= ""
	...
	if AlchemyFigureHead.selectedTabIdx == AlchemyFigureHeadTab.Upgrade then
		if (2 == itemContentsParam2) or (3 == itemContentsParam2) then
			msgContent = PAGetString(Defines.StringSheet_GAME, "LUA_ALCHEMYFISHPRINT_MSGBOX_CONTENT1")	-- "실패시 '바다의 토템'의 등급이 하락할 수 있습니다.\n이대로 진행하시겠습니까?"
		elseif (4 == itemContentsParam2) then
			msgContent = PAGetString(Defines.StringSheet_GAME, "LUA_ALCHEMYFISHPRINT_MSGBOX_CONTENT2")	-- "실패시 '바다의 토템'이 증발될 수 있습니다.\n이대로 진행하시겠습니까?"
...
function HandleClicked_AlchemyFigureHead_UnSetSlot( slotType )
	if 0 == slotType then
		...
		if false == AlchemyFigureHead.Stuff_slot.Empty then
			AlchemyFigureHead.Stuff_slot:clearItem()
			AlchemyFigureHead.Stuff_slot.Empty = true
...
function AlchemyFigureHead_FigureHeadRfunction( slotNo, itemWrapper, count, inventoryType )
	...
	local guideText = PAGetString(Defines.StringSheet_GAME, "LUA_ALCHEMYSTONE_SELECT_UPGRADE")	-- "블랙스톤(무기)를 선택해주세요."
	AlchemyFigureHead.control.guideString:SetText( guideText )
end
...
function FGlobal_AlchemyFigureHead_Open()
	local isAlchemyFigureHeadEnble = ToClient_IsContentsGroupOpen( CppEnums.ContentsGroupType.ContentsGroupKey, 37 )	-- 37 = 성장형 장비 컨텐츠('바다의 토템') 
	if not isAlchemyFigureHeadEnble then	-- 컨텐츠 옵션에서 열지 않으면 리턴!
		return
	end
	...
	AlchemyFigureHead:ClearData( true )		-- 초기화
	HandleClicked_AlchemyFigureHeadTab( 2 )	-- 탭 2 선택
...
-- function FGlobal_AlchemyFigureHead_Use()
	-- local itemWrapper = getEquipmentItem( CppEnums.EquipSlotNo.alchemyFigureHead )
	-- if nil ~= itemWrapper then
		-- useAlchemyFigureHead()
	-- end
-- end
...
function Panel_AlchemyFigureHead_updateTime( deltaTime )
	if AlchemyFigureHead.isPushDoit then
		...
		if 0 < elapsTime and false == AlchemyFigureHead.startEffectPlay then
			AlchemyFigureHead.Stuff_slot.icon:EraseAllEffect()
		...
		if 1 < elapsTime and false == AlchemyFigureHead.contentEffectPlay then
			-- 재료를 사라지게 한다.
			AlchemyFigureHead.Stuff_slot:clearItem()
			AlchemyFigureHead.Stuff_slot.Empty = true
		...
		if 2.5 < elapsTime and false == AlchemyFigureHead.slotEffectPlay then
			AlchemyFigureHead.FigureHead_slot.icon:EraseAllEffect()
			AlchemyFigureHead.FigureHead_slot.icon:AddEffect( "fUI_Alchemy_Stone_Upgrade02", false )
			AlchemyFigureHead.slotEffectPlay = true
		end

		if 3 < elapsTime and false == AlchemyFigureHead.effectEnd then
...
function FromClient_FigureHeadUpgrade( itemwhereType, slotNo, exp )
	...
	local mainMsg	= itemName .. PAGetString(Defines.StringSheet_GAME, "LUA_ALCHEMYSTONE_DONE_UPGRADE")	-- "itemName" .. " 성장 실패."
	...
	AlchemyFigureHead.resultMsg	= {}
	AlchemyFigureHead:ClearData()	-- 초기화
	Inventory_SetFunctor( AlchemyFigureHead_FigureHeadFilter, AlchemyFigureHead_FigureHeadRfunction, nil, nil )
end
...
function FromClient_FigureHeadChange( whereType, slotNo )
	...
	local itemName		= itemWrapper:getStaticStatus():getName()
	msg	= {
		main	= PAGetString(Defines.StringSheet_GAME, "LUA_ALCHEMYSTONE_MSG_DONE"),	-- "아이템 성장 성공"
...
function FromClient_FigureHeadChangeFailedByDown( whereType, slotNo )
	...
	local itemName		= itemWrapper:getStaticStatus():getName()
	msg	= {
		main	= itemName .. PAGetString(Defines.StringSheet_GAME, "LUA_ALCHEMYSTONE_DONE_UPGRADE"),	-- "itemName" .. " 성장 실패"
...
function FromClient_FigureHeadEvolve( evolveType )
	...
	if 1 == evolveType then
		Proc_ShowMessage_Ack( PAGetString(Defines.StringSheet_GAME, "LUA_ALCHEMYSTONE_MSG_LOST"))	-- "성장에 실패하여, 아이템이 증발됩니다."
	end

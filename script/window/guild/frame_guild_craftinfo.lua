...
function guildCraftInfo:exchangeSlotSet( exchange_Idx, isShow )					-- exchangeSlot on/off
	local exchangeSlot = self.uiPool.exchange[exchange_Idx]
...
function guildCraftInfo:Update()
	...
	for recipeArrayKey_Idx = self.config.scroll_startIdx, self.config.last_recipeArrayKey -1 do
		if 	self.config.maxExchangeCount-1 < exchangeUi_Idx then
			return
		end
		...
		if nil ~= guildHouseCraftInfo then
			...
			if itemExchangeSSW:isSet() then
				...
				for enchant_Idx = 0, exchangeCount -1 do
					...
					GuildCraft_ChangeBorder_ByItemGrade( gradeType, enchantSlot.border )	-- 그레이드에 따라 테두리 표현
					...
					if fullCount <= haveCount then
						enchantSlot.count:SetFontColor( UI_color.C_FF88DF00 )			-- 녹색
					else
						enchantSlot.count:SetFontColor( UI_color.C_FFE7E7E7 )			-- 흰색
					end
				end
				...
				exchangeSlot.border		:SetShow( false )	-- 등급 정보가 없다.
...
function Guild_CraftInfo_EnchantItemToolTip( isShow, enchantItemKeyRaw, exchangeUi_Idx, enchant_Idx )	-- 아이템 툴팁
	...
	if isShow then
		registTooltipControl( control, Panel_Tooltip_Item )	-- 툴팁이 남는 문제 수정용
...
function Guild_CraftInfo_ScrollEvent( isScrollUp )								-- 스크롤 이벤트
...
function FGlobal_Guild_CraftInfo_Init()	-- panel_guild.lua에서 실행.
	guildCraftInfo:Init()
end

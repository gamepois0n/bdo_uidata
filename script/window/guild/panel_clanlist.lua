...
function clanList_ShowAni()
	local aniInfo1 = Panel_ClanList:addScaleAnimation( 0.0, 0.08, UI_ANI_ADV.PAUI_ANIM_ADVANCE_COS_HALF_PI)
...
function clanList_HideAni()
	local aniInfo1 = Panel_ClanList:addColorAnimation( 0.0, 0.1, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
...
function clanList:Initialize()
	...
	self.btn_setSubMaster:SetText( PAGetString(Defines.StringSheet_GAME, "LUA_GUILD_TEXT_CLANLIST_SUBMASTER") )	-- "부대장 임명"
	...
	self.btn_kickClan:SetText( PAGetString(Defines.StringSheet_RESOURCE, "GUILD_BTN_FIRE") )	-- "추방"
	...
	self.btn_unsetSubMaster:SetText( PAGetString(Defines.StringSheet_GAME, "GULD_BUTTON3") ) -- "임명 해제" )	
	...
	for memberIdx = 0, self.maxMemberCount -1 do
		...
		Created_MemberGrade:SetText( "길드원" .. memberIdx )	
		...
		Created_MemberName:SetText( "멤버들이름이나옵니다(가문명이나오는곳입니)" )	
...
function clanList:Update()
	...
	if true == isGuildMaster then	
		self.btn_ClanDispersal		:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_GUILD_DISPERSE_CLAN")) -- "클랜 해산"
	else
		self.btn_ClanDispersal		:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_GUILD_WITHDRAW_CLAN")) -- "클랜 탈퇴"
	end
...
function MouseOutClanMenuButton()
	local self = clanList
...
function HandleClicked_ClanList_MemberMenu( memberIdx )
	...
	-- _PA_LOG("유흥신", "HandleClicked_ClanList_MemberMenu " ..memberIdx .." "..grade.." "..tostring(isGuildMaster).. " "..tostring(isGuildSubMaster) )
...
function HandleClicked_ClanList_KickClan()
	...
	local messageTitle	 	= PAGetString( Defines.StringSheet_GAME, "LUA_GUILD_TEXT_EXPEL_GUILDMEMBER" )
	local messageContent 	=	"'"..memberData:getName().."'".. PAGetString( Defines.StringSheet_GAME, "LUA_GUILD_TEXT_EXPEL_GUILDMEMBER_QUESTION" )
...
function HandleClicked_ClanList_UnsetSubMaster()
	...
	local messageTitle	 	= PAGetString( Defines.StringSheet_GAME, "LUA_GUILD_TEXT_APPOINT_GUILDMEMBER" )
	local messageContent 	=	"'"..memberData:getName().."'".. PAGetString( Defines.StringSheet_GAME, "LUA_GUILD_TEXT_APPOINT_GUILDMEMBER_QUESTION" )
...
function HandleClicked_ClanList_LeaveClan()
	...
	if true == getSelfPlayer():get():isGuildMaster() then
		...
		local tempText = PAGetString(Defines.StringSheet_GAME, "LUA_GUILD_TEXT_CLANLIST_CLANOUT_ASK") --'클랜에서 탈퇴 하시겠습니까?'

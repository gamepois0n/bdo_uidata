...
function	GuildHistory_DataUpdate()
	...
	ToClient_RequestJournalList( currentValue._year, currentValue._month, currentValue._guildHistory )							-- 일지 리스트 로드
end

local	_dayHistoryValue	=	{}
function	FromClient_GuildHistoryInfo_Update()
	_listCount	=	ToClient_GetJournalListCount( currentValue._year, currentValue._month, currentValue._guildHistory )		-- 해당 일지 수
...
function	HandleClicked_GuildHistory_MonthCheck( index )
	...
	if ( ToClient_GetThisMonth() == currentValue._month ) then			-- 현재 월을 클릭시(오늘 일자를 대입)
		...
		currentValue._day = pastMonth_DayCount[index+1]					-- 과거 월을 클릭시(과거 월의 마지막 일자를 대입)
	end
	
	GuildHistory_DataUpdate()											-- 일지 목록 갱신
end

...
function default_Control:UpdateList()
	...
	for idx = 0, lastIndex do
		...
		if false == House_SellBuy_Condition.isNext then
			checkButton = ( currentExplorePoint >= cost )	
			cost = " ( - " .. cost .. " )"
		else
			cost = " ( + " .. cost .. " )" 
		end
...
--WorldMapWindow.houseSellBuyCondition = WorldMapWindow.popupManager:createStackWindowDefault( Panel_House_SellBuy_Condition, false, 0 )

-- function WorldMapWindow.houseSellBuyCondition:updateFirst__()
	-- Panel_manageWorker:SetShow(false)
	-- Panel_Window_Warehouse:SetShow(false)	
	-- Panel_House_SellBuy_Condition:ComputePos()
-- end

-- function WorldMapWindow.houseSellBuyCondition:postHideUpdate__()
	-- clear_houseHoldSelected()
-- end
...
function check_SellBuyList(Input_houseKey)
	if false == House_SellBuy_Condition.isNext and 0 < #prevHouse_List then
		for idx, HouseKey in pairs (prevHouse_List) do
			if Input_houseKey == HouseKey then
				return true
			end
		end	
	elseif true == House_SellBuy_Condition.isNext and 0 < #nextHouse_List then
		for idx, HouseKey in pairs (nextHouse_List) do
			if Input_houseKey == HouseKey then
				return true
			end
...
function HandleClick_ListValueButton(idx)
	...
	-- WorldMapWindow_OnHouseholdClick(indexKey , true)
end

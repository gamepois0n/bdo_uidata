...
function	PetMarketShowAni()
	...
	-- 꺼준다
		...
		local aniInfo1 = Panel_Window_PetMarket:addColorAnimation( 0.0, 0.22, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
		...
		-- local aniInfo2 = Panel_Window_PetMarket:addScaleAnimation( 0.0, 0.22, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
...
local	PetMarket	= {
	...
	_mainBG						= UI.getChildControl( Panel_Window_PetMarket,	"Static_MainBG" ),				-- 슬롯이 들어갈 프레임

	_buttonQuestion				= UI.getChildControl( Panel_Window_PetMarket,	"Button_Question" ),			-- 물음표 버튼
	_buttonClose				= UI.getChildControl( Panel_Window_PetMarket,	"Button_Close" ),				-- 페이지 끄기
	
	_myMoneyValue				= UI.getChildControl( Panel_Window_PetMarket,	"Static_Text_Money" ),			-- 내가 가진 실버
		
	_staticPageNo				= UI.getChildControl( Panel_Window_PetMarket, 	"Static_PageNo" ),				-- 현제 페이지
	_buttonNext					= UI.getChildControl( Panel_Window_PetMarket, 	"Button_Next" ),				-- 다음 페이지
	_buttonPrev					= UI.getChildControl( Panel_Window_PetMarket,	"Button_Prev" ),				-- 전 페이지
	
	_buttonTabMarket			= UI.getChildControl( Panel_Window_PetMarket,	"RadioButton_List" ),			-- 시장 리스트
	_buttonTabMy				= UI.getChildControl( Panel_Window_PetMarket,	"RadioButton_ListMy" ),			-- 내 리스트
	
	_buttonBuy					= UI.getChildControl( Panel_Window_PetMarket,	"Button_Buy" ),					-- 구매 버튼
	_buttonCancel				= UI.getChildControl( Panel_Window_PetMarket,	"Button_Cancel" ),				-- 취소 버튼
	_buttonReceive				= UI.getChildControl( Panel_Window_PetMarket,	"Button_Receive" ),				-- 수령 버튼
	_buttonEnd					= UI.getChildControl( Panel_Window_PetMarket,	"Button_End" ),					-- 만료 버튼
...
function	PetMarket:registEventHandler()
	...
	self._buttonQuestion:addInputEvent(		"Mouse_LUp",	"Panel_WebHelper_ShowToggle( \"PanelWindowPetMarket\" )"			)			-- 물음표 좌클릭
	self._buttonQuestion:addInputEvent(		"Mouse_On",		"HelpMessageQuestion_Show( \"PanelWindowPetMarket\", \"true\")"	)			-- 물음표 마우스오버
	self._buttonQuestion:addInputEvent(		"Mouse_Out",	"HelpMessageQuestion_Show( \"PanelWindowPetMarket\", \"false\")"	)			-- 물음표 마우스아웃
...
function	PetMarket:registMessageHandler()
	registerEvent("FromClient_AuctionServantList",			"PetMarket_UpdateSlotData")
	registerEvent("FromClient_ServantRegisterToAuction",	"PetMarket_TabEventFromRegister")
	registerEvent("onScreenResize",							"PetMarket_Resize" )
end
...
function	PetMarket_Buy( slotNo )
	...
	local	messageboxData	= { title = titleString, content = contentString, functionYes = PetMarket_BuyXXX, functionCancel = MessageBox_Empty_function, priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW}

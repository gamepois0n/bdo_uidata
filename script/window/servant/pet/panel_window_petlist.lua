...
function	PetListShowAni()
	...
	if isShow == true then
		...
		local aniInfo1 = Panel_Window_PetList:addColorAnimation( 0.0, 0.22, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
		...
		-- local aniInfo2 = Panel_Window_PetList:addScaleAnimation( 0.0, 0.22, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
...
local	PetList	= {
	...
	_staticListBG			= UI.getChildControl( Panel_Window_PetList, 	"Static_ListBG"				),	-- list BackGround	
	_staticButtonListBG		= UI.getChildControl( Panel_Window_PetList, 	"Static_ButtonBG"			),	-- button BackGround
	_staticUnsealBG			= UI.getChildControl( Panel_Window_PetList, 	"Static_UnsealBG"			),	-- unseal Vehicle BackGround

	_staticNoticeText		= UI.getChildControl( Panel_Window_PetList, 	"StaticText_Notice" 		),	-- button List Notice ( 아무것도 없을 때 )
	_staticSlotCount		= UI.getChildControl( Panel_Window_PetList, 	"StaticText_Slot_Count"		),	-- 슬롯 개수 '1/5'
...
function	PetList:init()
	...
	--{
		...
		-- self._unseal._button:addInputEvent( "Mouse_LUp",	"PetList_UnsealSlotSelect()" )
	--}
...
function	PetList_ButtonOpen( eType, slotNo )
	...
	elseif ( eType == self._const.eTypeSealed ) then
		...
		-- if doneMating and servantInfo:isMale() then
		-- 	buttonList[button_Index]	= self._buttonReturnMating
		-- 	button_Index = button_Index + 1
		-- end
		if (nowMating ~= getState) and (regMating ~= getState) and (regMarket ~= getState) then
...
function	PetList_Button_Seal( isCompulsionSeal )
	...
	if	( isCompulsionSeal )	then
		...
		local messageboxData	= { title = messageBoxTitle, content = messageBoxMemo, functionYes = PetList_Button_CompulsionSeal, functionCancel = MessageBox_Empty_function, priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW}
...
function	PetList_Button_Recovery()
	...
	local	messageboxData	= { title = messageboxTitle, content = messageboxMemo, functionYes = confirmFunction, functionNo = MessageBox_Empty_function, priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW}
	MessageBox.showMessageBox(messageboxData)
...
function	PetList_Button_SellToNpc()
	...
	local	messageboxData	= { title = messageboxTitle, content = messageboxMemo, functionYes = PetList_Button_SellToNpcXXX, functionNo = MessageBox_Empty_function, priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW}
	MessageBox.showMessageBox(messageboxData)

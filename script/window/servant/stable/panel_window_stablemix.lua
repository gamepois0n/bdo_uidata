...
function	stableMix:update()
	...
	if ( nil ~= self._slotNo1 ) and ( nil ~= self._slotNo2 ) then
		...
		self._staticPrice:SetText( PAGetStringParam1( Defines.StringSheet_GAME, "LUA_STABLEMIX_MATINGPRICE", "matingServantPrice", makeDotMoney(matingServantPrice) ) ) -- "말 교환 비용은 : " .. makeDotMoney(matingServantPrice) .. " 은화 입니다.")
	end
...
function	StableMix_Mix()
	...
	local	messageBoxMemo = PAGetString(Defines.StringSheet_GAME, "LUA_STABLEMIX_MIXHELPMSG") -- "암말과 숫말을 마구간에서 새로운 말로 교환해줍니다."
...
function	FromClient_ServantMix( servantNo1, servantNo2 )
	...
	Proc_ShowMessage_Ack( PAGetStringParam2( Defines.StringSheet_GAME, "LUA_STABLEMIX_MIX_ACK", "servantName1", servantName1, "servantName2", servantName2 ) ) -- servantName1 .. " 탑승물과 " .. servantName2 .. " 탑승물이 교환되었습니다." )
...
function	StableMix_Resize()
	...
	--Panel_Window_StableMix:SetSize(screenX, screenY )

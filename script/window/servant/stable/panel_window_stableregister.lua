...
--Panel_Window_StableRegister:setGlassBackground(true)
--Panel_Window_StableRegister:RegisterShowEventFunc( true, '' )
--Panel_Window_StableRegister:RegisterShowEventFunc( false, '' )
...
local	stableRegister	= {	
	...
	_buttonQuestion						= UI.getChildControl( Panel_Servant_Market_Input,	"Button_Question" ),			-- 물음표 버튼
...
function	stableRegister:update()
	...
	if ( CppEnums.VehicleType.Type_Carriage == vehicleType ) or ( CppEnums.VehicleType.Type_CowCarriage == vehicleType ) then
		...
		self._staticMarketHp:SetText( PAGetString(Defines.StringSheet_GAME, "LUA_STABLEREGISTER_CARRIAGE_HP") ) -- "내구도" )
		self._staticMarketMp:SetText( PAGetString(Defines.StringSheet_GAME, "LUA_STABLEREGISTER_CARRIAGE_MP") ) -- "수명")
		self._staticMarketLife:SetText( PAGetString(Defines.StringSheet_GAME, "LUA_STABLEREGISTER_CARRIAGE_LIFE") ) -- "남은 기간" )
	else
		...
		self._staticMarketHp:SetText( PAGetString(Defines.StringSheet_GAME, "LUA_STABLEREGISTER_HP") ) -- "생명력" )
		self._staticMarketMp:SetText( PAGetString(Defines.StringSheet_GAME, "LUA_STABLEREGISTER_MP") ) -- "지구력")
		self._staticMarketLife:SetText( PAGetString(Defines.StringSheet_GAME, "LUA_STABLEREGISTER_LIFE") ) -- "남은 수명" )
	end
	...
	if	( (nil ~=self._minPrice) and (nil ~= self._maxPrice) )	then
		-- self._editEditMarketName:SetEditText( PAGetString(Defines.StringSheet_GAME, "LUA_STABLEREGISTER_PRICEGETEDIT") ) -- "가격을 입력해주세요." )
		...
		-- self._textMaxPriceValue:SetText( makeDotMoney(self._maxPrice) )
...
function	stableRegister:registEventHandler()
	...
	-- self._editEditMarketName:addInputEvent(		"Mouse_LUp",	"StableRegister_ClearEdit( 2 )"		)
	-- self._editEditMarketName:RegistReturnKeyEvent( "StableRegister_MarketRegister()" )
...
function	stableRegister:registMessageHandler()
	registerEvent("FromClient_ServantChildInfo",		"StableRegister_OpenByMating"	)	-- 망아지 수령할때 이벤트.
...
function	StableRegister_Register()
	...
	ClearFocusEdit()
	local	messageBoxMemo = PAGetStringParam1( Defines.StringSheet_GAME, "LUA_STABLEREGISTER_THISNAMEREGISTER", "name", name ) -- "<PAColor0xffd0ee68>" .. name .. "<PAOldColor>\n이 이름으로 등록하시겠습니까?"
...
function StableRegister_MarketRegister()
	local	self		= stableRegister
	-- if	( self._editEditMarketName:GetShow() and  (type(tonumber(self._editEditMarketName:GetEditText())) ~= "number") )	then
	-- 	Proc_ShowMessage_Ack( PAGetString(Defines.StringSheet_GAME, "LUA_STABLEREGISTER_PRICEGETEDIT_SERVANT_ACK") ) -- "탈 것의 가격을 입력해주세요.")
	-- 	return
	-- end
	...
	-- local	marketName	= self._editEditMarketName:GetEditText()
	...
	-- self._editEditMarketName:SetEditText("", true)
end
...
function	StableRegister_ClearEdit( clearIdx )
	...
	elseif clearIdx == 2 then
		-- self._editEditMarketName:SetEditText("", true)
		-- SetFocusEdit(self._editEditMarketName)
	end
...
function	StableRegister_Tab( tab )
	...
	if	(CppEnums.TransferType.TransferType_Normal == tab)	then
		...
		self._textSelfPrice:SetText( PAGetStringParam1( Defines.StringSheet_GAME, "LUA_STABLEREGISTER_MATING_PRICE", "matingPrice", tostring(getServantSelfMatingPrice()) ) ) -- "교배시 " .. tostring(getServantSelfMatingPrice()) .. "원이 차감됩니다." )
	end
end
...
function	StableRegister_Open()
	-- if	( Panel_Window_StableRegister:GetShow() )	then
	-- 	return
	-- end

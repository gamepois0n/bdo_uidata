	...
	-- { 아이템 아이콘 레벨 표기
		if 0 < enchantLevel and enchantLevel < 16 then
			if true == isCash then
				...
				self.ui.enchantLevel:SetText( "+" .. tostring(enchantLevel) )										-- 인챈트 수치
				self.ui.enchantLevel:SetShow(true)
			end
		elseif 16 == enchantLevel then
			self.ui.enchantLevel:SetText( "I" )		-- 인챈트 수치
			self.ui.enchantLevel:SetShow(true)
		elseif 17 == enchantLevel then
			self.ui.enchantLevel:SetText( "II" )		-- 인챈트 수치
			self.ui.enchantLevel:SetShow(true)
		elseif 18 == enchantLevel then
			self.ui.enchantLevel:SetText( "III" )		-- 인챈트 수치
			self.ui.enchantLevel:SetShow(true)
		elseif 19 == enchantLevel then
			self.ui.enchantLevel:SetText( "IV" )		-- 인챈트 수치
			self.ui.enchantLevel:SetShow(true)
		elseif 20 == enchantLevel then
			self.ui.enchantLevel:SetText( "V" )		-- 인챈트 수치
			self.ui.enchantLevel:SetShow(true)
		-- elseif 20 <= enchantLevel then
		-- 	self.ui.enchantLevel:SetText( "VI" )		-- 인챈트 수치
		-- 	self.ui.enchantLevel:SetShow(true)
		end
...
function ItemMarketAlarm_ShowAni()
	local aniInfo1 = Panel_ItemMarket_Alarm:addScaleAnimation( 0.0, 0.08, UI_ANI_ADV.PAUI_ANIM_ADVANCE_COS_HALF_PI)
...
function ItemMarketAlarm_HideAni()
	local aniInfo1 = Panel_ItemMarket_Alarm:addColorAnimation( 0.0, 0.1, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
...
function ItemMarketAlarm_CheckRestoreFlush()
	if ItemMarketAlarm.config.isOpen then
		Panel_ItemMarket_Alarm:SetShow( true, true )
	end
end
UI.addRunPostRestorFunction( ItemMarketAlarm_CheckRestoreFlush )

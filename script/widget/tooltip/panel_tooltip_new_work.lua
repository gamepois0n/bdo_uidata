...
local Insert_Tooltip_Work_Data = function(esSSW, isToolTip, controlIndex, nodeInfo, nodeType )
	...
	if ( nodeType == CppEnums.NpcWorkingType.eNpcWorkingType_PlantBuliding) then
		-- local workType = ToClient_getBuildingWorkableTypeByIndex(nodeInfo, workIndex)
		--[[
			local buildingStaticStatus			= ToClient_getBuildingWorkableBuildingSourceUnitByIndex(nodeInfo, workIndex)
...
function FGlobal_Hide_Tooltip_Work(esSSW, isReset)
	...
	if true ~= isReset then
		if esSSW == nil then
			return
		elseif esSSW ~= nil then
			if ( defalut_Param.esSS == esSSW:get() ) then
				return
			end
		end
	end
...
function FGlobal_Show_Tooltip_Work_Copy(esSSW, nodeInfo, nodeType )
	...
	for key = 0, defalut_Param.copyMaxCount - 1, 1 do
		if ( nil ~= defalut_Param.saveCopyIndex[key] ) then
			if ( defalut_Param.saveCopyIndex[key].esSS == esSSW:get() )then
				return
			end
		end		
	end

...
function FGlobal_ShowWorkerTooltip( workerData, uiBase )
	...
	controlWorkerTooltip._actionPointValue:SetText( tostring(workerData:getActionPoint()) .. " / " .. tostring(workerStaticStatus._actionPoint) )	 -- 액션포인트
	controlWorkerTooltip._luckValue:SetText( tostring(workerStaticStatus._luck / 10000) .. "%" ); -- 행운
	...
	controlWorkerTooltip._efficiencyValue:SetText(tostring( math.floor( _efficiency / 1000000 ) ) .. " /" .. _npcWorkingBaseTime)	 -- 작업 속도
	controlWorkerTooltip._moveSpeedValue:SetText( tostring(workerStaticStatus._moveSpeed) .. "m/s" ); -- 이동속도

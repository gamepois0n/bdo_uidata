...
--[[{
	...
	Type		= PIN_TYPE.PIN_LAST,		-- PIN Type
	
	CharacterNo	= 0,						-- UserID
}]]--
...
local Radar_Mine							-- 레이더의 자기 위치
local Pin_Party								-- 핀타입 : 파티
local Pin_Party_Mine						-- 핀타입 : 파티(자신)
local Pin_Guild								-- 핀타입 : 길드
local Pin_Guild_Mine						-- 핀타입 : 길드(자신)
local Pin_Guild_Master						-- 핀타입 : 길드마스터
local SelfPlayer							-- 내 캐릭터 정보
local Pin_GuideArrow
local Ping_Effect
...
local CalcRadarPostionToWorldPostion = function()
	...
	local intervalX = posX - ( Radar_Mine:GetPosX() + (Radar_Mine:GetSizeX() / 2))	-- 레이더 내의 포지션을 기준으로 좌표 변환
	...
	local float3Pos = float3(SelfPlayer:getPositionX() + intervalX,		-- X 좌표를 월드로 변환
							0,													-- Y는 쓰지 않는다
							SelfPlayer:getPositionZ() + intervalZ	)		-- Z 좌표를 월드로 변환
...
function FromClient_ResponseSendPositionGuideToRadar( PinInfo )	
	...
	if( PinInfo._isPermanent == false) then
		...
		if( pinPosX < 0 or pinPosX > RadarSizeX * 0.93 or
			pinPosY < 0 or pinPosY > RadarSizeY * 0.93 ) then
			return;
		else
			...
			if( PinInfo._isForGuild ) then
				...
				guideParam._color		= float4(1.0, 0.8, 0.6, 1.0);
				guideParam._bgColor		= float4(0.6, 0.2, 0.2, 0.3);
				guideParam._beamColor	= float4(0.4, 0.15, 0.15, 1.0);
...
function FromeClient_RadarPINCreate( PinInfo, control, pinGuideControl, isMine )	
	...
	ToClient_SetRadarCenterPos(float2(
					radarMap.pcPosBaseControl.x,
					radarMap.pcPosBaseControl.y))
end
...
function FGlobal_CommonPinUpdate( pos3D, pinControl, arrowControl, center )
	...
	if( pinPosX < 0 or pinPosX + pinControl:GetSizeX() > RadarSizeX * 0.93 or
		pinPosY < 0 or pinPosY  > RadarSizeY * 0.93 ) then
		...
		if center then
			pinControl:SetPosX(pinPosX - pinControl:GetSizeX() / 2.0);
		else
...
function FGlobal_UpdateGuildnPartyPin()
	...
	PositionGuideClear()
	local positionGuide = nil;
	local pinUI, pinGuideUI
...
--		local	intervalX = ( position.x - selfPlayerPosX ) * RadarPixelRate
--		local 	intervalZ = ( selfPlayerPosZ - position.z ) * RadarPixelRate
...
--		local pinPosY = intervalZ + ( RadarSizeY / 2 );
		...
		--	if( pinPosX < 0 or pinPosX + pinUI:GetSizeX() > RadarSizeX * 0.93 or
		--		pinPosY < 0 or pinPosY  > RadarSizeY * 0.93 ) then
	
		--		pinUI:SetShow(false);
		--		
		--		local dx = selfPlayerPosX - position.x

		--		local dy = selfPlayerPosZ - position.z;

		--		local radian = math.atan2( dx, dy );

		--		local guidePosX = RadarSizeX - pinGuideUI:GetSizeX()
		--		local guidePosY = RadarSizeY - pinGuideUI:GetSizeY()

		--		pinGuideUI:SetPosX( guidePosX / 2 )
		--		pinGuideUI:SetPosY( guidePosY / 2 )
		--		pinGuideUI:SetShow( true );
		--		pinGuideUI:SetRotate( radian );
		--		pinUI:SetShow(false)
		--	else
		--		pinUI:SetPosX(pinPosX);
		--		pinUI:SetPosY(pinPosY - pinUI:GetSizeY());
		--		pinUI:SetShow( true );
		--		pinGuideUI:SetShow(false)
		--	end
	end
end

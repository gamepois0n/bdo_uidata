...
local displayTime = function(timeValue)
	...
	elseif	toUint64(0,120) < timeValue then
	...
	elseif	toUint64(0,0) < timeValue then
...
local ConsignmentSaleManager = 
{
	...
	_buttonQuestion 		= UI.getChildControl(Panel_Housing_ConsignmentSale, "Button_Question"),			-- 물음표 버튼
}
--[[
ConsignmentSaleManager
	initialize()
	SetShow()
	UpdateData()
	UpdateMyData()
	UpdateDataCommon()
	Close()
	_topList : rtConsignment 검색해서 맴버 확인 요망
	_saleList : rtConsignment 검색해서 맴버 확인 요망	
]]--
...
function ConsignmentSaleManager:initialize()
	...
	Panel_Housing_ConsignmentSale:SetChildIndex( self._comboSellDate, Panel_Housing_ConsignmentSale:GetChildSize())	-- 컨트롤 우선순위 설정
	...
	self._buttonQuestion:addInputEvent( "Mouse_LUp", "Panel_WebHelper_ShowToggle( \"HousingConsignmentSale\" )" )	-- 물음표 좌클릭
	self._buttonQuestion:addInputEvent( "Mouse_On", "HelpMessageQuestion_Show( \"HousingConsignmentSale\", \"true\")" )	-- 물음표 마우스오버
	self._buttonQuestion:addInputEvent( "Mouse_Out", "HelpMessageQuestion_Show( \"HousingConsignmentSale\", \"false\")" )	-- 물음표 마우스아웃
	...
	copyStaticBack = nil
	copyButtonBuy = nil			
	copyTextBuyPrice = nil
	copyTextRegisterPrice = nil	
	copyStaticRemainedTime = nil				
	copyStaticItemicon = nil		
	copyStaticItemName = nil
	copyStaticRegisterPrice = nil
	copyStaticBuyPrice = nil	
	copyIconRegister = nil
	copyIconSale = nil
...
function executeConsignmentRetrieveSale() 
	ToClient_RequestRetrieveFromConsignmentSale(ConsignmentSaleManager._currentIndex)	-- 회수
...
-- 적용하기 버튼
--[[
function HandleClickedConsignmentApply()
	...
	if  0 < minRegisterPrice then
		ToClient_RequestSettingToConsignmentSale(selectCommissionIndex, selectSellDateIndex+1, toInt64(0, minRegisterPrice ) )
		ToClient_RequestGetConsignmentSaleInfo()
		ToClient_RequestRecontactInstallation()
		
		messageboxData = { title = "", content = PAGetString(Defines.StringSheet_GAME, "LUA_CONSIGNMENTSALE_APPLY_MESSAGE_1" ), functionApply = MessageBox_Empty_function, priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW }
	elseif minRegisterPrice <= 0 then
...
--[[	등록금 수정 부분
	if ConsignmentSaleManager._isShowMyList then
		ConsignmentSaleManager._saleList[index]._buttonBuy:SetText("등록금 수정")
		local consignmentData = ToClient_RequestConsignmentSaleListAt( index )
			
		local registerGold = toInt64(0, tonumber( ConsignmentSaleManager._saleList[index]._editRegisterPrice:GetEditText() ) )
		local saleGold = toInt64(0, tonumber ( ConsignmentSaleManager._saleList[index]._textBuyPrice:GetText() ) )
		UI.debugMessage("등록금 수정 "..Int64toInt32(consignmentData:getItem():get():getCount_s64()).." "..ConsignmentSaleManager._saleList[index]._editRegisterPrice:GetEditText().." "..ConsignmentSaleManager._saleList[index]._textBuyPrice:GetText() )
	end
	]]--
end
...
-- 에디트 박스 선택하고 esc 누를시 해제
function FGlobal_CheckCurrentConsignmentSaleUiEdit(targetUI)	-- globalKeyBinder.lua에서 사용
	local checkUi = false
...
function FGlobal_ConsignmentSaleClearFocusEdit()	-- globalKeyBinder.lua에서 사용
...
-- 인벤토리 우클릭 아이템 등록
function FGlobal_ConsignmentRegisterItemFromInventory(count, slotNo)	--Panel_Window_Inventory.lua 에서 사용됨
...
function handleClickedShowMessageConsignmentRegister()
	...
	if registerGold < ConsignmentSaleManager._registGoldMinimum then
	...
	local commission = toInt64(0, registerGold)
	...
	if (0.2 > (registerGold / saleGold)) then 
...
function executeConsignmentRegister()
	...
	ToClient_RequestRegisterToConsignmentSale(ConsignmentSaleManager._slotNo, ConsignmentSaleManager._itemCount, toInt64(0, registerGold), toInt64(0, saleGold))
	ToClient_RequestRecontactInstallation()
...
function handleClickedConsignmentWithdrawGold()
	...
	if gold <= 0 then
		...
		return
	elseif ConsignmentSaleManager._currentHoldingMoney < gold then
...
--				위탁 판매기 업데이트
----------------------------------------------------------------------------------------------------------------------
function ConsignmentSaleManager:UpdateData(isOwner)	-- 상품목록
	...
	for index = 0, 9, 1 do
		...
		elseif 1 == index then
			self._saleList[index]:SetData(nil)	-- 수수료 등등 여러 정보 들어가는 칸
		else	
			local salesindex = 0
			if 1 < index then	
	...
	--	ConsignmentSaleManager._saleList[index]._editRegisterPrice:SetShow(false)	-- 등록금 수정 관련
	--	ConsignmentSaleManager._saleList[index]._textRegisterPrice:SetShow(true)
	end
end

function ConsignmentSaleManager:UpdateMyData(isOwner)	-- 내목록
	...
	--	ConsignmentSaleManager._saleList[index]._editRegisterPrice:SetShow(true)	-- 등록금 수정관련
	--	ConsignmentSaleManager._saleList[index]._textRegisterPrice:SetShow(false)
...
function ConsignmentSaleManager:UpdateDataCommon(isOwner)	--  공통 부분
	...
	if true == self._isShowMyList then	-- 내 목록을 보고 있을 경우
	...
	if true == isOwner then
		ToClient_RequestGetHoldingMoney()	-- 잔고 업데이트	
	end
end
...
function ConsignmentSaleManager:SetRegistrableItemClassify()
	...
	if itemClassifyCount <= 1 then
		...
		--str = FGlobal_GetInteractionName()
		self._title:SetText(str)
		return
	end
	local count = 0
	
	for i=0, itemClassifyCount-1, 1 do
		local itemClassify = ToClient_RequestGetRegistrableItemAt(i)
		if -1 ~= itemClassify then
			if 0 == itemClassify then
				str = PAGetStringParam1( Defines.StringSheet_GAME, "LUA_HOUSINGCONSIGNMENTSALE_ETC", "str", str ) -- str .. "잡화"
				count = count + 1
			elseif 1 == itemClassify then
				str = PAGetStringParam1( Defines.StringSheet_GAME, "LUA_HOUSINGCONSIGNMENTSALE_WEAPON", "str", str ) -- str .. "무기"
				count = count + 1
			elseif 2 == itemClassify then
				str = PAGetStringParam1( Defines.StringSheet_GAME, "LUA_HOUSINGCONSIGNMENTSALE_ARMOR", "str", str ) -- str .. "방어구"
				count = count + 1
			elseif 3 == itemClassify then
				str = PAGetStringParam1( Defines.StringSheet_GAME, "LUA_HOUSINGCONSIGNMENTSALE_ACCESSORIES", "str", str ) -- str .. "악세사리"	
				count = count + 1
			end
			
			if (0 < i) and (i < count-1) then
				str = str .. ", "
			end
		end
	end
	...
	if 0 == count then
		str = ''
	elseif 1 == count then
		str = PAGetString(Defines.StringSheet_GAME, "LUA_HOUSINGCONSIGNMENTSALE_CONSIGNMENT") -- str .. " 위탁 판매소"
	elseif 4 == count then
		str = PAGetString(Defines.StringSheet_GAME, "LUA_HOUSINGCONSIGNMENTSALE_CONSIGNMENT_ALL") -- "위탁 판매소(전체)"
	else
		str = PAGetStringParam1( Defines.StringSheet_GAME, "LUA_HOUSINGCONSIGNMENTSALE_CONSIGNMENT_ETC", "str", str ) -- "위탁 판매소(" .. str .. ")"		
	end
...
function ConsignmentSaleManager:Close()		
	if Panel_Housing_ConsignmentSale:IsShow() then
		...
		ClearFocusEdit()
		FGlobal_UpdateInventorySlotData() -- Panel_Window_Inventory
		ConsignmentSaleManager._slotNo 		= 0
		ConsignmentSaleManager._itemCount 	= 0
	end
end
...
registerEvent("FromClient_EventShowHoldingMoneyManager", 		"FromClient_EventShowConsignmentHoldingMoneyManager")	-- 잔고 표시용

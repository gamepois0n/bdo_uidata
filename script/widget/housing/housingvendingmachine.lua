...
local VendingMachineManager =
{
	...
	_buttonQuestion		= UI.getChildControl(Panel_Housing_VendingMachineList, "Button_Question"),			-- 물음표 버튼
...
function VendingMachineManager:initialize()
	...
	function createVendingMechineList( pIndex )
		...
		local slot = {}
		SlotItem.new( slot, "ItemIcon_" .. pIndex, pIndex, rtMachine._staticItemIcon, _vendingMachineSlotConfig )	-- 외부에서 불러오는 함수
		slot:createChild()	-- 외부에서 불러오는 함수
		...
		function rtMachine:SetData(vendingData)
			...
			if vendingData:isEmpty() then
				...
				if Int64toInt32(vendingData._itemCount) <= 0 then
					...
					rtMachine._buttonOut:SetShow(true and VendingMachineManager._isOwner)
					rtMachine._buttonBuy:SetShow(true and not VendingMachineManager._isOwner)
				...
				if true == ToClient_RequestBusinessIsActive() then
					...
					if 0 < Int64toInt32(vendingData._itemCount) then
						rtMachine._buttonDeleteItem:SetShow(false)	
					else
	...
	self._buttonQuestion:addInputEvent( "Mouse_LUp", "Panel_WebHelper_ShowToggle( \"HosingVendingMachine\" )" )	-- 물음표 좌클릭
	self._buttonQuestion:addInputEvent( "Mouse_On", "HelpMessageQuestion_Show( \"HosingVendingMachine\", \"true\")" )	-- 물음표 마우스오버
	self._buttonQuestion:addInputEvent( "Mouse_Out", "HelpMessageQuestion_Show( \"HosingVendingMachine\", \"false\")" )	-- 물음표 마우스아웃	
	...
	copyTextSellPrice = nil
	copyTextItemCount = nil
	copyTextMaxCount = nil
	copyStaticItemIcon = nil
	copyStaticItemName = nil
	copyStaticBuyPrice = nil
	copyStaticSellPrice = nil
	copyStaticMaxCount = nil
	copyEditBuyPrice = nil
	copyEditSellPrice = nil
	copyEditMaxCount = nil
	copyButtonDeleteItem = nil
	...
	if false == ToClient_RequestBusinessIsActive() then
		for index = 0, 3, 1 do	-- 가격값 우선 체크
			...
			if 0 < VendingMachineManager._list[index]._checkSlotIn then
				if maxCount <= 0 or buyPrice <= 0 or sellPrice <= 0 then
		...
		for index = 0, 3, 1 do	-- 체크 통과시 등록함
			local maxCount = VendingMachineManager._list[index]._editMaxCount:GetEditNumber()
			local buyPrice = VendingMachineManager._list[index]._editBuyPrice:GetEditNumber()
			local sellPrice = VendingMachineManager._list[index]._editSellPrice:GetEditNumber()
		
			if 2 == VendingMachineManager._list[index]._checkSlotIn then	-- 마우스 우클릭으로 들어온 아이템 있을 경우
			...
			elseif 3 == VendingMachineManager._list[index]._checkSlotIn then
				...
				if  maxCount < Int64toInt32(vendingData._itemCount) then
					Proc_ShowMessage_Ack( PAGetString( Defines.StringSheet_GAME, "LUA_VENDINGMACHINE_STARTSTOP_MESSAGE_2" ) )
					return
				end

				if false == vendingData:isEmpty() then	-- 슬롯이 비어있지 않을 경우
		...
		if false == checkSlot then	-- 물품이 하나도 등록이 안될 경우 판매 시작 불가
...
function handleClickedVendingMachinePerform()
	...
	if gold <= 0 then
		...
		return
	elseif VendingMachineManager._currentHoldingMoney < gold and 1 == VendingMachineManager._checkFlag then
...
-- 인벤토리 우클릭 아이템 등록
function FGlobal_VendingMachineRegisterItemFromInventory(slotNo)	--Panel_Window_Inventory.lua 에서 사용됨	
	local slotIndex = -1
	...
	if true == VendingMachineManager._isOwner then
		if false == ToClient_RequestBusinessIsActive() then
			...
			--if itemWrapper:getStaticStatus():isEquipable() or itemWrapper:get():isVested() or false == itemWrapper:getExpirationDate():isIndefinite() then
			if (not itemWrapper:getStaticStatus():isStackable()) or 
				itemWrapper:get():isVested() or 
				false == itemWrapper:getExpirationDate():isIndefinite() or
				itemWrapper:getStaticStatus():isForJustTrade()
				then
...
function handleClickedVendingMachineIconReset(index)
	...
	if 0 < itemCount and false == vendingData:isEmpty() then
...
function FGlobal_CheckCurrentVendingMachineUiEdit(targetUI)	-- globalKeyBinder.lua에서 사용
	local checkUi = false
...
function FGlobal_VendingMachineClearFocusEdit(targetUI)	-- globalKeyBinder.lua에서 사용
	for index = 0, 3, 1 do
		local vendingData = ToClient_RequestVendingMachineListAt(index)
		if targetUI:GetKey() == VendingMachineManager._list[index]._editBuyPrice:GetKey() then	
			if vendingData:isEmpty() then
...
function VendingMachineManager:UpdateData( isOwner )
	...
	for index = 0, (vendingMachineListCount - 1), 1 do
		local vendingData = ToClient_RequestVendingMachineListAt(index)

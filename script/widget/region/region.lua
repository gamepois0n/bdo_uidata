...
-- Panel Init
---------------------------
Panel_Region:SetShow(false, false)			-- Default Hide!
...
-- 패널 따로
	...
	-- -- _PA_LOG( 'Lua Debug', 'call RegionShow' )
	local aniInfo = Panel_Region:addColorAnimation( 0.0, 1.05, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
	aniInfo:SetStartColor( UI_color.C_00FFFFFF )
	aniInfo:SetEndColor( UI_color.C_FFFFFFFF )
	aniInfo.IsChangeChild = false

	-- 텍스트 따로
	local aniInfo2 = regionName:addColorAnimation( 0.0, 1.05, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
	aniInfo2:SetStartColor( UI_color.C_00FFFFFF )
	aniInfo2:SetEndColor( UI_color.C_FFFFFFFF )
	aniInfo2.IsChangeChild = false

	local aniInfo3 = regigonPlace:addColorAnimation( 0.0, 1.05, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
	aniInfo3:SetStartColor( UI_color.C_00FFFFFF )
	aniInfo3:SetEndColor( UI_color.C_FFFFFFFF )
	aniInfo3.IsChangeChild = false
	
	local aniInfo4 = siegeHouse:addColorAnimation( 0.0, 1.05, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
	aniInfo4:SetStartColor( UI_color.C_00FFFFFF )
	aniInfo4:SetEndColor( UI_color.C_FFFFFFFF )
	aniInfo4.IsChangeChild = false

	local aniInfo5 = L_Line:addColorAnimation( 0.0, 1.5, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
	aniInfo5:SetStartColor( UI_color.C_00FFFFFF )
	aniInfo5:SetEndColor( UI_color.C_FFFFFFFF )
	aniInfo5.IsChangeChild = false

	local aniInfo6 = R_Line:addColorAnimation( 0.0, 1.5, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
	...
	if isShowAlert  then
		...
		local aniInfo_alsertImg = alertBackImg:addColorAnimation( 0.0, 1.05, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
		...
		local aniInfo_alertText = alertText:addColorAnimation( 0.0, 1.05, UI_ANI_ADV.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
...
function Region_HideAni()
	...
	local aniInfo6 = Panel_Region:addColorAnimation( 0.0, 1.5, UI_ANI_ADV.PAUI_ANIM_ADVANCE_COS_HALF_PI)
...
function Region_OnChangeRegion( regionData )
	...
	if nil == regionData then
		...
		regionName:SetText ( regionData:getAreaName() )				-- 지역명 출력
		regigonPlace:SetText ( regionData:getTerritoryName() )		-- 영지명 출력
		...
		-- if ( UI_RT.eRegionType_Siege == regionType ) then
		-- 	siegeHouse:SetShow( true )
		-- 	siegeHouse:SetText( PAGetString( Defines.StringSheet_GAME, "LUA_REGION_POSSIBLE_COMMAND_POST") )
			
		-- elseif ( UI_RT.eRegionType_Fortress == regionType ) then
		-- 	siegeHouse:SetShow( true )
		-- 	siegeHouse:SetText( PAGetString( Defines.StringSheet_GAME, "LUA_REGION_POSSIBLE_FORTESS") )
		...
		-- regionName:SetText ( tostring ( regionData:getNationalName() ) )			-- ?
		-- UI.debugMessage(tostring(regionData:getAreaName() ) )

		-- regionData:getTerritoryName()
		-- regionData:getAreaName()
		-- regionData:getNationalName()
	end
end

...
-- Panel_Command:RegisterShowEventFunc( true, 'HudTimerShowAni()' )
------------------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------------------------
...
local isCommandFinished = true			-- 커맨드 게임이 끝났느냐!

-- 생선의 등급을 골라냄
local fishCommandCount = {
	[0] = { min = 2, max = 3 },			-- 일반 = 2 ~ 4개
	{ min = 4, max = 5 },				-- 매직 = 4 ~ 5개
	{ min = 5, max = 6 },				-- 레어 = 5 ~ 6개
	{ min = 7, max = 9 },				-- 유니크 = 7 ~ 9개
	{ min = 10, max = 10 },				-- 에픽 = 10개
}

local fishCount = -1
local FishGrade = -1
local getFishCountCalc = function()
	FishGrade = getSelfPlayer():get():getFishGrade()	-- 잡은 물고기 등급 알아오기
	--local FishGrade = 3
	local discountCommand = 0
	local fishLevel = getSelfPlayer():get():getLifeExperienceLevel( 1 )			-- 낚시 레벨을 가져오자
	...
	if ( FishGrade < 5 ) then
		...
		--_PA_LOG( "asdf", "fishLevel : " .. tostring( fishLevel ) .. "discountCommand : " .. tostring( discountCommand ) .. " minCount : " .. tostring( minCount ) .. " maxCount : " .. tostring( maxCount ) .. " dice100 : " .. tostring( dice100 ) )
	else
		UI.ASSERT(false, "그레이드가 잘못 들어왔습니다. 루아에도 물고기 그레이드를 바꿔주세요.")
	end
end
...
local passedTimePerNext = 0		-- 한번 누른 이후 지난 시간
...
local Command_CreateRandomText = function()
	...
	for index = 1, fishCount do
		...
		textControl:SetColor( UIColor.C_FFEFEFEF )	-- 색은 여기서 바꾸지 않음.
...
function Panel_Minigame_Command_Start()
	...
	Command_RePosition()	-- 포지션을 변화한다.

	FGlobal_MiniGame_FishingCheck()

	getFishCountCalc()
	isCommandFinished = false
		_commandEffectBG:AddEffect( "UI_Fishing_Aura02", false, 0, 0 ) -- 창기씨 낚시 커맨드 이펙트 수정부탁드려요
	...
	local colorNo		= math.random( 0, 108 )	-- CommandColor에 정의된 109개 중 하나의 컬러를 쓴다.
	local randomAlpha	= string.format( "%.2f", (math.random( 75, 100 )/100))

	for index = 1, 10 do	-- 여기서 색 조절
...
local MiniGame_Command_OnSuccess = function()
	...
	getSelfPlayer():get():SetMiniGameResult( 2 )		-- Success !!!
	Panel_Command:RegisterUpdateFunc("")
...
local MiniGame_Command_OnFailed = function()
	...
	getSelfPlayer():get():SetMiniGameResult( 0 )		-- FAIL!!
	isCommandFinished = true
	Panel_Command:RegisterUpdateFunc("")
	-- UI.debugMessage( 'Failed!' )
	textControl:SetColor( UIColor.C_FFFF0000 )		-- 틀린것도 색을 바꿔준다!
	textControl:SetFontColor( UIColor.C_FFFF0000 )		-- 틀린것도 색을 바꿔준다!
...
function MiniGame_Command_ddukdition( keyType )
	...
	--_PA_LOG("LUA", 'Command Good Key Type ' .. tostring(keyType) )
	...
	if commands[currentCommandIndex] == keyType then
		...
		textControl:SetColor( UIColor.C_FF88DF00 )		-- 맞춘건 색을 바꿔준다!
		textControl:SetFontColor( UIColor.C_FF88DF00 )		-- 맞춘건 색을 바꿔준다!
...
function Command_UpdateText( deltaTime )
	...
	-- UI.debugMessage('rate : ' .. rate )
	
	-- 3초씩 주어진다!
	if ( 6 < passedTimePerNext ) then
		MiniGame_Command_OnFailed()
	end
end

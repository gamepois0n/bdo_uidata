...
local mgUI = { base = {}, select = {}, tooltip = {}, left={}, center={}, right={}, zodiac={}, }

local giftIcon = {}
-- index : 0 ~ count
	-- uiControl : giftIcon UI


local gameStep = 0
-- 0 화면만 떠있음(게임중이 아닐수도 있음)
-- 1 이야기 교류가 가능한 상황
-- 2 이야기 교류중
-- 3 이야기 교류 끝
-- 4 이야기 교류 나가기 버튼 눌렀을때

local mouseInputer = nil			-- MouseInput받는 Panel UI
local isRdown = false				-- R Button Down시에 true
local rMovePos = { x = 0 , y = 0 }	-- R Dwon 시에 이전 좌표

local addIntimacy = 0

local _bestPoint = 0			-- 최대 호감 점수
local hideDeltaTime = 0			-- gameStep == 4일때 지난 시간
local endTimechk = 0			-- 게임 나가기 화면에서 꺼지는 시간 카운트
local gamePlayCount = 0			-- 게임을 시작한 횟수
...
local mentalGame_End = false	-- 자동 종료 체크용
...
local uiInit = function()
	...
	-- base.apply				= UI.getChildControl(Panel_MentalGame_Base, "Button_Apply")
...
local registEventInit = function()
	...
	local base = mgUI.base
	-- base.apply:addInputEvent("Mouse_LDown",			"MentalKnowledge_Apply_LClick()")
...
local createCenterUI = function()
	...
	local createUI = function( key, uiGroup )
		...
		local panel = UI.createPanel( "Panel_MentalGame_Select_" .. tostring(key), Defines.UIGroup.PAGameUIGroup_QuestLog )
		panel:SetAlpha(1.0)
		panel:SetSize(10, 10)
		...
		--panel:addInputEvent("Mouse_LDown",			"MentalKnowledge_ClearCard(" .. key .. ")")
	...
	for from = 0, 19 do
		...
		for to = from + 1, 19 do
			...
			if ( nil == targetGroup.panel ) then
				local target = UI.createPanel( "MentalGame_Animation_" .. tostring(from) .."_".. tostring(to), Defines.UIGroup.PAGameUIGroup_QuestLog )
				--CopyBaseProperty( select.panel, target )
...
local insertCircleLineAndObject = function()
	...
	MentalKnowledgeBase.color = float4( 30 / 255, 209 / 255, 253 / 255, 0.3 )
	MentalKnowledgeBase.arrowColor = float4( 30 / 255, 209 / 255, 253 / 255, 0.3 )
	...
	MentalKnowledgeBase.color = float4( 0.83, 0.79, 0.54, 1.0 )
	MentalKnowledgeBase.arrowColor = float4( 0.83, 0.79, 0.54, 1.0 )
	local prevIndex = -1
	...
	MentalKnowledgeBase.UpdateLineAndCircle() --원과 선의 위치를 맞추어줌.
end
...
local insertCardList = function()
	...
	for index = 0, count - 1 do
		...
		if ( nil == rightCardList[index].ui.cardText ) then
			...
			--aControl[index]:addInputEvent("Mouse_LUp",				"MentalKnowledge_OnLUp(" .. index .. " )")
...
local updateCardColor = function()
	--rightCardList[index].ui.cardText
	local mentalObject = getMentalgameObject()
	if nil == mentalObject then
		return
	end
...
local updateStateUIShow = function()
	...
	if ( 0 == gameStep ) then -- 0 화면만 떠있음(게임중이 아닐수도 있음)
	...
	elseif ( 1 == gameStep ) then -- 1 이야기 교류가 가능한 상황
		left.fruitage_Add		:SetShow(true)
		base.comment_1			:SetShow(true)
		...
		center.finishImage		:SetShow(false)
		center.panel			:SetShow(false)
	elseif ( 2 == gameStep ) then -- 2 이야기 교류중
		left.fruitage_Add		:SetShow(true)
		...
		center.finishImage		:SetShow(false)
		center.panel			:SetShow(false)
	elseif ( 3 == gameStep ) then -- 3 이야기 교류 끝
	...
	elseif ( 4 == gameStep ) then
		...
		local aniInfo1 = center.finishImage:addScaleAnimation( 0.0, 0.16, UI_ANI_ADV.PAUI_ANIM_ADVANCE_COS_HALF_PI)
		...
		local aniInfo3 = center.finishText:addScaleAnimation( 0.0, 0.16, UI_ANI_ADV.PAUI_ANIM_ADVANCE_COS_HALF_PI)
...
local updateState = function()
	...
	elseif(goaltype == 1)	then  -- 흥미 유발 횟수
		local temp1 = ""
	...
	elseif(goaltype == 3)	then  -- 최대 호감도
		...
		base.condition:SetText(temp1)
	elseif(goaltype == 4)	then -- 흥미 유발 실패
	...
	if ( base.condition:GetShow() ) then
		...
		-- base.condition:SetPosX( leftPos )
	end
	...
	if ( 0 < comboCount and gameStep == 2) then
		...
		-- left.circle:SetShow(true)
		left.circle:ResetVertexAni()
		left.circle:SetPosX(left.comboCount_1:GetPosX() - 3)
		left.circle:SetPosY(left.comboCount_1:GetPosY() - 3)
		-- left.circle:SetPosX(left.comboCount_1:GetPosX() + left.comboCount_1:GetTextSizeX() / 2 - left.circle:GetSizeX() / 2 + 3 )
		-- left.circle:SetPosY(left.comboCount_1:GetPosY() + left.comboCount_1:GetSizeY() / 2 - left.circle:GetSizeY() / 2 )
		-- left.circle:SetVertexAniRun( "Ani_Scale_Circle0", true )
		-- left.circle:SetVertexAniRun( "Ani_Scale_Circle1", true )
		-- left.circle:SetVertexAniRun( "Ani_Scale_Circle2", true )
		-- left.circle:SetVertexAniRun( "Ani_Color_Circle0", true )
		-- left.circle:SetVertexAniRun( "Ani_Color_Circle1", true )
		-- left.circle:SetVertexAniRun( "Ani_Color_Circle2", true )
		
		-- left.circle2:SetShow(true)
		left.circle2:ResetVertexAni()
		left.circle2:SetPosX(left.comboCount_1:GetPosX())
		left.circle2:SetPosY(left.comboCount_1:GetPosY())
		-- left.circle2:SetPosX(left.comboCount_1:GetPosX() + left.comboCount_1:GetTextSizeX() / 2 - left.circle2:GetSizeX() / 2 + 3 )
		-- left.circle2:SetPosY(left.comboCount_1:GetPosY() + left.comboCount_1:GetSizeY() / 2 - left.circle2:GetSizeY() / 2 )
		-- left.circle2:SetVertexAniRun( "Ani_Scale_Circle0", true )
		-- left.circle2:SetVertexAniRun( "Ani_Scale_Circle1", true )
		-- left.circle2:SetVertexAniRun( "Ani_Scale_Circle2", true )
		-- left.circle2:SetVertexAniRun( "Ani_Color_Circle0", true )
		-- left.circle2:SetVertexAniRun( "Ani_Color_Circle1", true )
		-- left.circle2:SetVertexAniRun( "Ani_Color_Circle2", true )
	end
	...
	elseif(goaltype == 0)	then 	-- 게임 방법 설명
		base.explain:SetShow(false)
	elseif(goaltype == 1)	then	-- 흥미 유발 횟수
	...
	-- base.explain:SetPosX(base.condition:GetPosX() + 30)
...
local updateCardScrollButton = function()
	...
	right.cardRightArrow:SetShow( 0 < scrollPositionResult )
end
...
function MentalGame_Show()
	...
	SetUIMode(Defines.UIMode.eUIMode_MentalGame)	-- 더 좋은 방법이 있기 전까지는 여기에서 UI Mode 를 변경해준다! - 성경
	...
	--다른 UI 꺼주기 ( 추가적인것들을 넣던지, 안넣어도 되게 하자 )
	...
	HandleMLUp_SkillWindow_Close( true )		-- true 이면 ani 를 하지 않는다!
	NpcShop_WindowClose()
	------------------------
...
function MentalGame_ScreenResize()
	...
	-- leftPanel:SetPosX( emptySize * 13 / 27 ) 캐릭터옆 정보 박스 원래 위치
	...
	--local fullwidth = mgUI.left.panel:GetPosX()
	--
	--getMentalgameObject():setCharacterCreatePosition( fullwidth / sizeX / 2, 0.5, fullwidth / sizeX )
end
	...
	if ( gameStep < 2 ) then
		if ( mouseOverKey == overKey ) then
			...
			if ( isShow ) then
				...
				if ( isSuccess ) then
					tooltip.panel:SetShow(true, false)	-- 이야기 교류 툴팁 창을 보인다.
					overKey = mouseOverKey
					if ( nil ~= targetUI ) then

						-- local panelPos = targetUI:GetPosY() + targetUI:GetSizeY()
						-- if ( getScreenSizeY() < panelPos + tooltip.panel:GetSizeY() ) then
						-- 	panelPos = targetUI:GetParentPosY() - tooltip.panel:GetSizeY()
						-- end
						-- tooltip.panel:SetPosY( panelPos )
						-- tooltip.panel:SetPosY( aControl:GetPosY() -300 )

						-- 툴팁 위치는 부모 아이콘 왼쪽 위로 나온다.
						tooltip.panel:SetPosX( targetUI:GetParentPosX() - ( tooltip.panel:GetSizeX() - 40 ) )	-- 툴팁 위치는 하단 아이콘왼쪽으로 나온다.
						tooltip.panel:SetPosY( targetUI:GetParentPosY() - ( tooltip.panel:GetSizeY() - 50 )  )	-- 툴팁 위치는 하단 아이콘 위치에서 툴팁 세로 크기 - 50 만큼 뺀 값이다.
					end
				end
			end
		end
...
function MentalKnowledge_UpdateCenterSlot( key )
	...
	if ( selectCardIndex < mentalObject:getCardCount() ) and (nil ~= value) then
		...
		RequestMentalGame_clearSelectCard(key)	-- key 를 누르면 초기화 시킨다.
		local staticKey = value:getStaticStatus():getKey()
	...
	-- 클릭한 지점에 카드가 nil 이 아니면, 빼는 사운드를 재생한다.
...
local updateNextTryEvent = function()
	...
	if ( 0 < nextSlot ) then
		...
		--이전 슬롯의 성공여부를 이펙트로 표시한다.
		local index = mentalObject:getOrder(prevSlot)
		--centerUiList[index].circularProgress:SetCurrentControlPos(0)
		--centerUiList[index].circularProgress:SetProgressRate(100)
		...
		if ( mentalObject:isComboSuccess() ) then
			...
			-- "실패!"
			audioPostEvent_SystemUi(04,08)
		end

		local lastIndex = mentalObject:getOrderCount() -1
		local nextIndex = mentalObject:getOrder(nextSlot)

		--다음슬롯의 효과프로그래스바를 활성화 시킨다.
		if ( mentalObject:getHasNextSlot() ) then
			if ( nil ~= centerUiList[nextIndex] ) then
...
function MentalGame_StateUpdate( isNext )
	...
	-- mentalObject:scaling(-0.2)	-- 지식 집어넣을 때 마다 크기가 줄어들어서 삭제함
end
...
function MentalGame_UpdateEndTimer( deltaTime )
	...
	if 5 < endTimechk and true == mentalGame_End then
		MentalGame_Hide()
		mentalGame_End = false
	end

	if 6 < endTimechk then
		endTimechk = 0
	end
end

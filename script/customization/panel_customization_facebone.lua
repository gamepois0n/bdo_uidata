...
StaticText_DefaultControl:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CUSTOMIZATION_SELECT_CONTROL"))	-- 컨트롤러 종류
StaticText_Symmetry:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CUSTOMIZATION_SYMMETRY"))	-- 좌우 대칭
StaticText_ControlPart:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CUSTOMIZATION_CONTROLPART"))	-- 전체 초기화
...
--Button_Apply:addInputEvent("Mouse_LUp",		"applyControlParameters()")
...
--registerEvent("EventSetControlParameters", "SetControlParameters")
...
--registerEvent("EventRepositioningGroupUI", "RepositioningGroupUI")
...
local ControlMode = 1; -- 1:Trans , 2:Rotate, 3:Scale
...
function InitializeCustomizationData( customizationData , _selectedCharacterClass )
	...
	--addMotionGroupData( customizationData, _selectedCharacterClass )
	
	--addExpressionGroupData( customizationData, _selectedCharacterClass )
	
	--addClothGroupData( customizationData, _selectedCharacterClass )
	
	InitializePoseEditor( customizationData )
	
	--sculpting bone 용 테스트 UI 배치 , 나중에 지워야 함.
...
function SelectSculptingBoneControl( customizationData )
	...
	local vector = {x, y, z}
	--vector = customizationData:getSelectedBonePickPos()
	--self.EditText[ 2 ]:SetEditText( floatString( vector.x ) )
	--self.EditText[ 3 ]:SetEditText( floatString( vector.y ) )
	--self.EditText[ 4 ]:SetEditText( floatString( vector.z ) )
...
-- sculpting bone 세팅을 위해 필요 정보 xml에 세이브 하는 함수
...
function RepositioningGroupUI( groupIndex, posX, posY )
	--GroupTree[ groupIndex ]:getRootItem().control:SetSpanSize(posX, posY)
end

function RepositionCursorUI( posX, posY )
--	if bOpenSculptingUI == true then
		--Static_MouseCursor[ ControlMode ]:SetPosX( posX - 42 )
		--Static_MouseCursor[ ControlMode ]:SetPosY( posY - 42 )
		--CheckCursorInSculptingUI()
--	end
end
...
function CursorSelect( luaControlModeIndex )
	--Static_MouseCursor[ControlMode]:SetShow( false )
	ControlMode = luaControlModeIndex
	--Static_MouseCursor[ControlMode]:SetShow( true )
...
function EnableCursor( Enable )
	--Static_MouseCursor[ControlMode]:SetShow( Enable )
	--if true == Enable then
--		setCursorMode(0)
--	else
		--setCursorMode(1)

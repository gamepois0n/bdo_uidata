...
--staticPoseImage[1] = UI.getChildControl( Panel_CustomizationMainSlide, "Static_facial")
...
function SlideMenuFilter( index )
	--menuFilter = index
	menuFilter = 5
	--slideButtons[index].control:SetShow( false )
	--slideButtons[index].slideImage:SetShow( false )
end
...
local SlideMenuCloseMain = function()
	...
	for idx=1, 4 do
		...
		if idx ~= menuFilter then
			...
			local aniInfo = control:addMoveAnimation( 0.0, 0.1, CppEnums.PAUI_ANIM_ADVANCE_TYPE.PAUI_ANIM_ADVANCE_LINEAR )
			local mainEndPosX = (idx-1 + filterIndex) * control:GetSizeX()
			local imageControl = slideButtons[idx].slideImage
			local aniImageInfo = imageControl:addMoveAnimation( 0.0, 0.1, CppEnums.PAUI_ANIM_ADVANCE_TYPE.PAUI_ANIM_ADVANCE_LINEAR )
			...
			local aniImageColorInfo = imageControl:addColorAnimation(0.0, 0.2, CppEnums.PAUI_ANIM_ADVANCE_TYPE.PAUI_ANIM_ADVANCE_LINEAR )
			...
			for subidx=1, slideButtons[idx].subControlCount do
				...
				local subAniInfo = UIAni.AlphaAnimation( 0, subControl, 0.0, 0.2 )
				...
				if( nil ~= subControlImage ) then
					local aniSubInfo = UIAni.AlphaAnimation( 0, subControlImage, 0.0, 0.2 )
			...
			local alpha = imageControl:GetAlpha()
				if idx == index then
					...
					for subidx=1, slideButtons[index].subControlCount do
						...
						UIAni.AlphaAnimation( 1, subControl, 0.0, 0.2 )
						
						local subAniInfo = subControl:addMoveAnimation( 0.0, 0.2, CppEnums.PAUI_ANIM_ADVANCE_TYPE.PAUI_ANIM_ADVANCE_LINEAR )
						...
						if( subControlImage ~= nil ) then
							...
							UIAni.AlphaAnimation( 1, subControlImage, 0.0, 0.2 )
							
							local subImageAniInfo = subControlImage:addMoveAnimation( 0.0, 0.2, CppEnums.PAUI_ANIM_ADVANCE_TYPE.PAUI_ANIM_ADVANCE_LINEAR )
					...
					for subidx=1, slideButtons[idx].subControlCount do
						local subControl = slideButtons[idx].subControl[subidx]
						local aniInfo = UIAni.AlphaAnimation( 0, subControl, 0.0, 0.2 )
...
function CustomizationSlideUIShow( show )
	...
	if( show == true ) then
		...
		UIAni.AlphaAnimation( 1, Panel_CustomizationMainSlide, 0.0, 0.2 )
		...
		local aniInfo = UIAni.AlphaAnimation( 0, Panel_CustomizationMainSlide, 0.0, 0.2 )
		aniInfo:SetHideAtEnd(true)
	end
end